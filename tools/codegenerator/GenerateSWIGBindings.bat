...
"%bin_dir%\swig\swig.exe" -w401 -c++ ^
  -outdir "%python_generated_dir%" ^
  -o "%python_generated_dir%\%~2.xml" ^
  -xml ^
  -I"%base_dir%\xbmc" ^
  "%swig_dir%\%~2.i"
...
"%java_exe%" ^
  -cp "%groovy_dir%\groovy-all-2.4.4.jar;%groovy_dir%\commons-lang-2.6.jar;%generator_dir%;%python_dir%" ^
  groovy.ui.GroovyMain ^
  "%generator_dir%\Generator.groovy" ^
  "%python_generated_dir%\%~2.xml" ^
  "%python_dir%\PythonSwig.cpp.template" ^
  "%python_generated_dir%\%~2.cpp" ^
  "%doxygen_dir%"

rem delete the XML file generated by SWIG as it's not needed anymore
IF EXIST "%python_generated_dir%\%~2.xml" del "%python_generated_dir%\%~2.xml" >NUL

rem delete the generated doxygen files
IF EXIST "%doxygen_dir%" rmdir "%doxygen_dir%" /S /Q >NUL
