...
{
  m_vecList->Add(boost::make_shared<CFileItem>(item));
}
...
void CGUIDialogColorPicker::LoadColors(const std::string& filePath)
{
  ...
  {
    for (std::vector<std::pair<std::string, UTILS::COLOR::ColorInfo> >::iterator color = colors.begin(); color != colors.end(); ++color)
    {
    ...
    CLog::Log(LOGERROR, "%s - An error occurred when loading colours from the xml file.",
              __FUNCTION__);
}
...
void CGUIDialogColorPicker::OnInitWindow()
{
  if (!m_vecList || m_vecList->IsEmpty())
    CLog::Log(LOGERROR, "%s - No colours have been added in the list.", __FUNCTION__);
