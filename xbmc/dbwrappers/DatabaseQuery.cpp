...
{
  if (!obj.isObject() || !obj.isMember("field") || !obj["field"].isString() ||
...
CDatabaseQueryRule::SEARCH_OPERATOR CDatabaseQueryRule::TranslateOperator(const char* oper)
{
  for (unsigned int i = 0; i < NUM_OPERATORS; i++)
    if (StringUtils::EqualsNoCase(oper, operators[i].string))
...
std::string CDatabaseQueryRule::GetLocalizedOperator(SEARCH_OPERATOR oper)
{
  for (unsigned int i = 0; i < NUM_OPERATORS; i++)
    if (oper == operators[i].op)
      return g_localizeStrings.Get(operators[i].localizedString);
...
CDatabaseQueryRuleCombination::CDatabaseQueryRuleCombination() : m_type(CombinationAnd)
{
}
...
bool CDatabaseQueryRuleCombination::Save(CVariant& obj) const
{
  ...
  {
    for (CDatabaseQueryRuleCombinations::const_iterator combo = m_combinations.begin();
         combo != m_combinations.end(); ++combo)
    {
