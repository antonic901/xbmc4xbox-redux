...
std::string CDatabase::GetSingleValue(const std::string& query,
                                      boost::movelib::unique_ptr<Dataset>& ds)
{
  std::string ret;
  try
  {
...
bool CDatabase::CommitMultipleExecute()
{
  m_multipleExecute = false;
  BeginTransaction();
  for (std::vector<std::string>::const_iterator i = m_multipleQueries.begin();
       i != m_multipleQueries.end(); ++i)
  {
    if (!ExecuteQuery(*i))
    {
      RollbackTransaction();
...
void CDatabase::InitSettings(DatabaseSettings& dbSettings)
{
...
#endif
  {
    ...
    if (dbSettings.host.empty())
      dbSettings.host =
          CSpecialProtocol::TranslatePath(CProfilesManager::Get().GetDatabaseFolder());
  }
