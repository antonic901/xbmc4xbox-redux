...
{
  // Open the local file
  CAutoPtrHandle hFile(CreateFile(strFilename.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING,
  ...
  // Read the header
  DWORD dwRead;
  if (!::ReadFile((HANDLE)hFile, m_pHeader,
                  std::min(m_iHeaderSize, (int)GetFileSize((HANDLE)hFile, (LPDWORD)NULL)), &dwRead,
                  NULL))
  {
    return false;
  }
  ...
  m_XBEInfo.pSection_Header =
      (_xbe_info_::section_header*)(m_pHeader + (m_XBEInfo.Header.section_headers_addr -
  ...
  while (iNumSections-- > (int)0)
  {
    ...
    if ((int)m_XBEInfo.pSection_Header->Flags.inserted_file == (int)1)
    {
      ...
      pszSectionName =
          (m_pHeader + (m_XBEInfo.pSection_Header->section_name_addr - m_XBEInfo.Header.base));

      // Title image?
      if (strcmp(pszSectionName, "$$XTIMAGE") == (int)0)
      {
        // Position at 'title image' in the XBE itself
        if (SetFilePointer((HANDLE)hFile, m_XBEInfo.pSection_Header->raw_addr, NULL, FILE_BEGIN) ==
            (DWORD)0xFFFFFFFF)
        {
        ...
        // Read the image
        if (::ReadFile((HANDLE)hFile, m_pImage,
                       std::min(m_iImageSize, (int)m_XBEInfo.pSection_Header->sizeof_raw), &dwRead,
                       NULL) == (BOOL)FALSE)
        {
        ...
        CAutoPtrHandle hIcon(
            CreateFile(strIcon.c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, 0, NULL));
        if (!hIcon.isValid())
          return false;
...
uint32 CXBE::ExtractGameRegion(const CStdString& strFilename)
{
  ...
  if (SetFilePointer((HANDLE)hFile, m_XBEInfo.Header.certificate_addr - m_XBEInfo.Header.base, NULL,
                     FILE_BEGIN) == (DWORD)0xFFFFFFFF)
  {
    return (0);
  }
  // re-read to be sure
  if (!::ReadFile((HANDLE)hFile, m_pHeader, sizeof(m_XBEInfo.Certificate), &dwRead, NULL))
  {
    return (0);
  }
...
int CXBE::FilterRegion(int iRegion, bool bForceAllModes)
{
  ...
  else if ((iRegion & 4 && iPreferred == 4) ||
           (iRegion ==
            4)) // stored as 4 in the db (xbe values) but the actual video-mode is VIDEO_PAL50=3
    iRegion = VIDEO_PAL50;
  else if (iRegion == 8)
    iRegion = VIDEO_PAL60;
  else if (iRegion & 1)
