...
BOOL XKHDD::SendATACommand(UCHAR DeviceNum, LPATA_COMMAND_OBJ ATACommandObj, UCHAR ReadWrite)
{
...
#if defined(_WINDOWS)
  HANDLE Device = CreateFile(tmp, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
  ...
  status = NtOpenFile(&Device, SYNCHRONIZE | GENERIC_READ | GENERIC_WRITE, &oa, &iosb,
...
#endif
  LPATA_PASS_THROUGH pAPT =
      (LPATA_PASS_THROUGH)VirtualAlloc(NULL, Size, MEM_COMMIT, PAGE_READWRITE);
  ...
  if (pAPT)
  {
    ...
    BOOL Status = DeviceIoControl(Device, IOCTL_IDE_PASS_THROUGH, pAPT, Size, pAPT, Size,
                                  &BytesReturned, FALSE);
