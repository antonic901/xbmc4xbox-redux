...
bool CVideoInfoScanner::DoScan(const std::string& strDirectory)
{
  ...
  const std::vector<std::string>& regexps = content == CONTENT_TVSHOWS
                                                ? g_advancedSettings.m_tvshowExcludeFromScanRegExps
                                          ...
                                          CGUIDialogProgress* pDlgProgress)
{
  ...
  for (int i = 0; i < (int)items.Size(); ++i)
  {
    ...
    if (IsExcluded(pItem->GetPath(), (content == CONTENT_TVSHOWS)
                                         ? g_advancedSettings.m_tvshowExcludeFromScanRegExps
                                         : g_advancedSettings.m_moviesExcludeFromScanRegExps))
      continue;
                                                                ...
                                                                CGUIDialogProgress* pDlgProgress)
{
  ...
  if (GetDetails(pItem, url, info2,
                 (result == CNfoFile::COMBINED_NFO || result == CNfoFile::PARTIAL_NFO)
                     ? &m_nfoReader
                     : NULL,
                 pDlgProgress))
  {
...
bool CVideoInfoScanner::EnumerateSeriesFolder(CFileItem* item, EPISODELIST& episodeList)
{
  ...
  if (item->m_bIsFolder)
  {
    ...
    if (!bSkip)
    {
      ...
      CUtil::GetRecursiveListing(item->GetPath(), items, g_advancedSettings.m_videoExtensions,
                                 flags);
  ...
  for (int i = 0; i < items.Size(); ++i)
  {
    ...
    if (!EnumerateEpisodeItem(items[i].get(), episodeList))
      CLog::Log(LOGDEBUG, "VideoInfoScanner: Could not enumerate file %s",
                CURL::GetRedacted(CURL::Decode(items[i]->GetPath())).c_str());
  }
  return true;
}
                                 ...
                                 bool libraryImport /* = false */)
{
  ...
  CLog::Log(LOGDEBUG, "VideoInfoScanner: Adding new item to %s:%s",
            TranslateContent(content).c_str(), redactPath.c_str());
  long lResult = -1;
                                   ...
                                   const std::string& actorArtPath)
{
  ...
  // add online art
  for (std::vector<CScraperUrl::SUrlEntry>::const_iterator it =
           pItem->GetVideoInfoTag()->m_strPictureURL.m_url.begin();
       it != pItem->GetVideoInfoTag()->m_strPictureURL.m_url.end(); ++it)
  {
    ...
    bool useLocal)
{
  ...
  // add online art
  for (std::vector<CScraperUrl::SUrlEntry>::const_iterator it = show.m_strPictureURL.m_url.begin();
       it != show.m_strPictureURL.m_url.end(); ++it)
  {
    const CScraperUrl::SUrlEntry& i = *it;
...
CNfoFile::NFOResult CVideoInfoScanner::CheckForNFOFile(CFileItem* pItem,
                                                       bool bGrabAny,
                                                       ScraperPtr& info,
                                                       CScraperUrl& scrUrl)
{
...
int CVideoInfoScanner::FindVideo(const std::string& videoName,
                                 const ScraperPtr& scraper,
                                 CScraperUrl& url,
                                 CGUIDialogProgress* progress)
{
