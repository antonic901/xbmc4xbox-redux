...
void CAsyncGetItemsForPlaylist::GetItemsForPlaylist(const boost::shared_ptr<CFileItem>& item)
{
  ...
  if (item->m_bIsFolder)
  {
    if (!item->IsPlugin())
    {
      ...
      if (!mediapath.empty())
      {
        m_queuedItems.Add(boost::make_shared<CFileItem>(mediapath, false));
        return;
      }
    }
    ...
    if (m_resume)
    {
      ...
      boost::shared_ptr<CFileItem> lastPlayedItem;
      ...
      for (int j = 0; j < items.Size(); ++j)
      {
        ...
        CVideoInfoTag *const videoTag = i->GetVideoInfoTag();
  ...
  else if (item->IsVideoDb())
  {
    ...
    const CFileItemPtr itemCopy = boost::make_shared<CFileItem>(*item->GetVideoInfoTag());
...
bool IsAutoPlayNextItem(const std::string& content)
{
  ...
  const boost::shared_ptr<CSettingList> setting = boost::dynamic_pointer_cast<CSettingList>(
...
void PlayItem(
    const boost::shared_ptr<CFileItem>& itemIn,
    const std::string& player,
    ContentUtils::PlayMode::Type mode /* = ContentUtils::PlayMode::CHECK_AUTO_PLAY_NEXT_VIDEO */)
{
  ...
  {
    AddItemToPlayListAndPlay(item, CFileItemPtr(), player);
  }
...
bool IsItemPlayable(const CFileItem& item)
{
  ...
  if (item.IsPlayList())
  {
    ...
    if (StringUtils::StartsWith(item.GetPath(), StringUtils::Format("%s/video/", path.c_str())) ||
        StringUtils::StartsWith(item.GetPath(), StringUtils::Format("%s/mixed/", path.c_str())))
      return true;
  ...
  else
  {
    ...
    if (item.IsVideoDb() || item.IsDVD())
    {
      ...
      else if (item.IsVideoDb())
      {
        ...
        long id = -1;
        VideoDbContentType::Type content_type;
        ...
        {
          CLog::Log(LOGERROR, "Cannot obtain video content type");
          db.Close();
          return ResumeInformation();
        }
      ...
      {
        // DVD
        CLog::Log(LOGERROR, "Cannot obtain bookmark for DVD");
        db.Close();
        return ResumeInformation();
      }
    }
    ...
    if (resumeInfo.startOffset > 0)
    {
      std::string resumeString = StringUtils::Format(
          ...
          StringUtils::SecondsToTimeString(
              ...
              TIME_FORMAT_HH_MM_SS).c_str());
      if (resumeInfo.partNumber > 0)
      {
        const std::string partString =
            StringUtils::Format(g_localizeStrings.Get(23051).c_str(), resumeInfo.partNumber);
