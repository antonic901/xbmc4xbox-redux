...
SelectFirstUnwatchedItem CGUIWindowVideoNav::GetSettingSelectFirstUnwatchedItem()
{
  if (m_vecItems->IsVideoDb())
  {
    ...
    {
      int iValue = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt("videolibrary.tvshowsselectfirstunwatcheditem");
...
{
  int iValue = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt("videolibrary.tvshowsincludeallseasonsandspecials");
  if (iValue >= NEITHER && iValue <= SPECIALS)
...
void CGUIWindowVideoNav::SelectFirstUnwatched() {
  ...
  if (selectFirstUnwatched != SelectFirstUnwatchedItem::NEVER)
  {
      ...
      (selectFirstUnwatched == SelectFirstUnwatchedItem::ON_FIRST_ENTRY && !bIsItemSelected))
    {
      ...
      bool bIncludeAllSeasons = (incAllSeasonsSpecials == BOTH || incAllSeasonsSpecials == ALL_SEASONS);
      bool bIncludeSpecials = (incAllSeasonsSpecials == BOTH || incAllSeasonsSpecials == SPECIALS);
...
bool CGUIWindowVideoNav::GetDirectory(const std::string &strDirectory, CFileItemList &items)
{
  ...
  if (bResult)
  {
    if (items.IsVideoDb())
    {
      ...
      int iFlatten = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt("videolibrary.flattentvshows");
...
void CGUIWindowVideoNav::LoadVideoInfo(CFileItemList &items, CVideoDatabase &database, bool allowReplaceLabels)
{
  ...
  /*
    ...
    * whether or not we assign the library's labels to the item, or leave the item as is.
    ...
    setting is enabled.
    */
  const bool stackItems    = items.GetProperty("isstacked").asBoolean() || (StackingAvailable(items) && CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool("myvideos.stackvideos"));
  const bool replaceLabels = allowReplaceLabels && CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool("myvideos.replacelabels");
  ...
  for (int i = 0; i < items.Size(); i++)
  {
    ...
    else
    {
      ...
      if (pItem->IsVideo())
        pItem->SetOverlayImage(pItem->GetVideoInfoTag()->m_playCount > 0 ? CGUIListItem::ICON_OVERLAY_WATCHED : CGUIListItem::ICON_OVERLAY_UNWATCHED);
    }
  }
}
...
void CGUIWindowVideoNav::OnDeleteItem(CFileItemPtr pItem)
{
  ...
  else if (m_vecItems->GetContent() == "tags" && pItem->m_bIsFolder)
  {
    CGUIDialogYesNo* pDialog = (CGUIDialogYesNo*)CServiceBroker::GetGUI()->GetWindowManager().GetWindow(WINDOW_DIALOG_YES_NO);
    pDialog->SetHeading(432);
    pDialog->SetLine(1,  StringUtils::Format(g_localizeStrings.Get(433).c_str(), pItem->GetLabel().c_str()) );
    pDialog->SetLine(2, "");
    pDialog->Open();
    if (pDialog->IsConfirmed())
    {
  ...
  case CONTEXT_BUTTON_PLAY_OTHER:
    {
      ...
      if (database.GetSong(database.GetSongByArtistAndAlbumAndTitle(StringUtils::Join(m_vecItems->Get(itemNumber)->GetVideoInfoTag()->m_artist, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_videoItemSeparator),m_vecItems->Get(itemNumber)->GetVideoInfoTag()->m_strAlbum,
                                                                        ...
                                                                        song))
      {
        CServiceBroker::GetAppMessenger()->PostMsg(TMSG_MEDIA_PLAY, 0, 0, static_cast<void*>(new CFileItem(song)));
      }
      return true;
    }
