...
CVideoTagLoaderPlugin::CVideoTagLoaderPlugin(const CFileItem& item, bool forceRefresh)
  : IVideoInfoTagLoader(item, ADDON::ScraperPtr(), false), m_force_refresh(forceRefresh)
{
  if (forceRefresh)
    return;
  ...
  if (m_item.HasVideoInfoTag())
    m_tag = boost::movelib::make_unique<CVideoInfoTag>(*m_item.GetVideoInfoTag());
  const CGUIListItem::ArtMap &art = item.GetArt();
  if (!art.empty())
    m_art = boost::movelib::make_unique<CGUIListItem::ArtMap>(art);
}

bool CVideoTagLoaderPlugin::HasInfo()
{
...
CInfoScanner::INFO_TYPE CVideoTagLoaderPlugin::Load(CVideoInfoTag& tag, bool, std::vector<EmbeddedArt>*)
{
  if (m_force_refresh)
  {
    ...
    if (!items.IsEmpty())
    {
      ...
      m_art = boost::movelib::make_unique<CGUIListItem::ArtMap>(item->GetArt());
