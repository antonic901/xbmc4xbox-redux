...
{
  { 630,   ViewModeNormal, false, false },
  { 631,   ViewModeZoom, false, false },
  { 632,   ViewModeStretch4x3, false, false },
  { 633,   ViewModeStretch14x9, false, false },
  { 634,   ViewModeStretch16x9, false, false },
  { 635,   ViewModeOriginal, false, false },
  { 636,   ViewModeCustom, HIDE_ITEM, false }
};
 ...
 */
void CViewModeSettings::ViewModesFiller(const boost::shared_ptr<const CSetting>& setting,
                                        std::vector<IntegerSettingOption>& list,
                                        int& current,
                                        void* data)
{
  // Add all appropriate view modes to the list control
  for (size_t i = 0; i < NUMBER_OF_VIEW_MODES; i++)
  {
    if (!viewModes[i].hideFromList)
      list.push_back(IntegerSettingOption(g_localizeStrings.Get(viewModes[i].stringIndex), viewModes[i].viewMode));
  }
}
