...
std::string CGUISliderControl::GetDescription() const
{
  ...
  {
    if (m_rangeSelection)
      description = StringUtils::Format("[%i%%, %i%%]", MathUtils::round_int(static_cast<double>(m_percentValues[0])),
                                        MathUtils::round_int(static_cast<double>(m_percentValues[1])));
    else
      description = StringUtils::Format("%i%%", MathUtils::round_int(static_cast<double>(m_percentValues[0])));
  }
  return description;
}
