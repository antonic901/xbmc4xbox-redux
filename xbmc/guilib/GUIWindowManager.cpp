...
void CGUIWindowManager::PreviousWindow()
{
  ...
  CServiceBroker::GetGUI()->GetInfoManager().SetNextWindow(previousWindow);

  // deinitialize our window
  CloseWindowSync(pCurrentWindow);

  CServiceBroker::GetGUI()->GetInfoManager().SetNextWindow(WINDOW_INVALID);
  CServiceBroker::GetGUI()->GetInfoManager().SetPreviousWindow(currentWindow);
  ...
  CServiceBroker::GetGUI()->GetInfoManager().SetPreviousWindow(WINDOW_INVALID);
  return;
}
...
void CGUIWindowManager::ActivateWindow_Internal(int iWindowID, const std::vector<std::string>& params, bool swappingWindows, bool force /* = false */)
{
  ...
  CServiceBroker::GetGUI()->GetInfoManager().SetNextWindow(iWindowID);
...
{
  m_tracker.MarkDirtyRegion(CRect(0, 0, (float)CServiceBroker::GetWinSystem()->GetGfxContext().GetWidth(), (float)CServiceBroker::GetWinSystem()->GetGfxContext().GetHeight()));
}
