                        ...
                        float maxPixelWidth)
{
  ...
  for (std::vector<UTILS::COLOR::Color>::const_iterator color = colors.begin(); color != colors.end(); ++color)
    renderColors.push_back(context.MergeColor(*color ? *color : m_textColor));
  if (!shadowColor)
