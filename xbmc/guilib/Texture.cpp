...
/*                                                                      */
/************************************************************************/
CBaseTexture::CBaseTexture(unsigned int width,
                           unsigned int height,
                           unsigned int format,
...
CBaseTexture* CBaseTexture::LoadFromFile(const CStdString& texturePath,
...
bool CBaseTexture::LoadFromFileInternal(const CStdString& texturePath,
                                        unsigned int maxWidth,
                                        unsigned int maxHeight,
                                        bool autoRotate)
{
  unsigned int width = maxWidth
                           ? std::min(maxWidth, (unsigned int)g_graphicsContext.GetMaxTextureSize())
                           : (unsigned int)g_graphicsContext.GetMaxTextureSize();
  unsigned int height =
      maxHeight ? std::min(maxHeight, (unsigned int)g_graphicsContext.GetMaxTextureSize())
                : (unsigned int)g_graphicsContext.GetMaxTextureSize();
    ...
    if (XFILE::CFile::Exists(ddsPath))
    {
      ...
      if (img.ReadFile(ddsPath))
      {
        ...
        */
        ...
        g_graphicsContext.Get3DDevice()->CreateTexture(img.GetWidth(), img.GetHeight(), 1, 0,
                                                       D3DFMT_DXT1, D3DPOOL_MANAGED, &m_texture);
        if (m_texture)
        {
          D3DLOCKED_RECT lr;
          if (D3D_OK == m_texture->LockRect(0, &lr, NULL, 0))
        ...
        {
          CLog::Log(LOGERROR, "%s - failed to create texture from dds image %s", __FUNCTION__,
                    ddsPath.c_str());
  ...
  if (URIUtils::HasExtension(texturePath, ".jpg|.tbn"))
  {
    ...
    if (jpegfile.Open(texturePath, width, height))
    {
      ...
      if (jpegfile.Width() > 0 && jpegfile.Height() > 0)
      {
        ...
        g_graphicsContext.Get3DDevice()->CreateTexture(
            ((jpegfile.Width() + 3) / 4) * 4, ((jpegfile.Height() + 3) / 4) * 4, 1, 0,
            D3DFMT_LIN_A8R8G8B8, D3DPOOL_MANAGED, &m_texture);
        if (m_texture)
        {
          D3DLOCKED_RECT lr;
          if (D3D_OK == m_texture->LockRect(0, &lr, NULL, 0))
  ...
  if (mimeType == "image/jpeg")
  {
    ...
    if (jpegfile.Read(buffer, size, maxWidth, maxHeight))
    {
      if (jpegfile.Width() > 0 && jpegfile.Height() > 0)
      {
        ...
        {
          CLog::Log(LOGERROR, "%s - failed to create texture while loading image from memory",
                    __FUNCTION__);
          return false;
        }
      }
...
void CBaseTexture::LoadFromImage(ImageInfo& image, bool autoRotate)
{
  ...
  g_graphicsContext.Get3DDevice()->CreateTexture(image.width, image.height, 1, 0,
                                                 D3DFMT_LIN_A8R8G8B8, D3DPOOL_MANAGED, &m_texture);
  if (m_texture)
  {
    D3DLOCKED_RECT lr;
                                ...
                                IDirect3DPalette8* palette)
{
  ...
  if (D3DXCreateTexture(g_graphicsContext.Get3DDevice(), w, h, 1, 0, D3DFMT_P8, D3DPOOL_MANAGED,
                        &m_texture) == D3D_OK)
  {
    D3DLOCKED_RECT lr;
