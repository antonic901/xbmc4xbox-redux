...
bool CGUIListContainer::OnAction(const CAction& action)
{
  switch (action.GetID())
  {
    ...
    {
      if (GetOffset() == (int)m_items.size() - m_itemsPerPage ||
  ...
  : CGUIBaseContainer(parentID, controlID, posX, posY, width, height, VERTICAL, 200, 0)
{
  ...
  focusLayout.CreateListControlLayouts(width, textureHeight + spaceBetweenItems, true, labelInfo,
                                       labelInfo2, textureButton, textureButtonFocus, textureHeight,
                                       itemWidth, itemHeight, condition2, condition);
