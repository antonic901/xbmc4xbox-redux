...
void GetArtwork(CFileItem* item)
{
  ...
  CFileItemList items;
  CDirectory::GetDirectory(strArtworkPath, items, g_advancedSettings.m_pictureExtensions,
                           DIR_FLAG_DEFAULTS);
  for (int i = 0; i < items.Size(); ++i)
  {
    ...
    if (StringUtils::EqualsNoCase(strFileName, "poster") ||
        StringUtils::EqualsNoCase(strFileName, "fanart"))
      item->SetArt(strFileName, pItem->GetPath());
  }

  for (CGUIListItem::ArtMap::const_iterator it = item->GetArt().begin(); it != item->GetArt().end();
       ++it)
...
bool CProgramInfoScanner::DoScraping(const std::string& strDirectory,
                                     ...
                                     int idPath /* = -1 */)
{
  ...
  CFileItemList items;
  if (!CDirectory::GetDirectory(strDirectory, items, g_advancedSettings.m_programExtensions,
                                DIR_FLAG_DEFAULTS))
    return false;
...
void CProgramInfoScanner::ScrapeProgram(const std::string& strPath,
                                        const ScraperPtr& scraper,
                                        int idPath)
{
