...
void CProgramDatabase::CreateTables()
{
  ...
  m_pDS->exec("CREATE TABLE path (idPath integer primary key, strPath text, strScraper text, "
              "dateAdded text)");
  ...
  m_pDS->exec("CREATE TABLE trainers (idTrainer integer primary key, idTitle integer, "
              "strTrainerPath text, strSettings text, Active integer)\n");
}
...
std::string CProgramDatabase::GetValueString(const CProgramInfoTag& details,
                                             int min,
                                             int max,
...
void CProgramDatabase::GetDetailsFromDB(const dbiplus::sql_record* const record,
                                        int min,
                                        int max,
      ...
      case PROGRAMDB_TYPE_STRINGARRAY:
      {
        ...
        if (!value.empty())
          *(std::vector<std::string>*)(((char*)&details) + offsets[i].offset) =
              StringUtils::Split(value, g_advancedSettings.m_programItemSeparator);
        break;
      }
      case PROGRAMDB_TYPE_DATE:
...
bool CProgramDatabase::AddTrainer(int idTitle, CTrainer& trainer)
{
  ...
  try
  {
    ...
    strSQL = PrepareSQL("insert into trainers (idTrainer, idTitle, strTrainerPath, strSettings, "
                        "Active) values (NULL, %u, '%s', '%s', %i)",
                        idTitle, trainer.GetPath().c_str(), temp, 0);
...
bool CProgramDatabase::SetTrainer(int idTitle, CTrainer* trainer)
{
  ...
  try
  {
    ...
    strSQL = PrepareSQL(
        "update trainers set Active=%u, strSettings='%s' where idTrainer=%i and idTitle=%u", 1,
        temp, trainer->GetTrainerId(), idTitle);
    m_pDS->exec(strSQL.c_str());
...
bool CProgramDatabase::GetTrainerOptions(int idTrainer,
                                         ...
                                         int numOptions)
{
  ...
  try
  {
    ...
    strSQL =
        PrepareSQL("select * from trainers where idTrainer=%i and idTitle=%u", idTrainer, idTitle);
    if (!m_pDS->query(strSQL.c_str()))
      return false;
...
bool CProgramDatabase::HasTrainer(const std::string& strTrainerPath)
{
  ...
  try
  {
    ...
    strSQL = PrepareSQL("SELECT strTrainerPath FROM trainers WHERE strTrainerPath='%s'",
                        strTrainerPath.c_str());
  ...
  try
  {
    ...
    strSQL =
        PrepareSQL("insert into path (idPath, strPath, dateAdded) values (NULL, '%s', '%s')",
                   strPath1.c_str(), CDateTime::GetCurrentDateTime().GetAsDBDateTime().c_str());
...
int CProgramDatabase::GetProgramId(const std::string& strFilenameAndPath)
{
  ...
  try
  {
    ...
    strSQL = PrepareSQL("select idProgram from program where c%02d='%s'", PROGRAMDB_ID_PATH,
...
int CProgramDatabase::AddProgram(const std::string& strFilenameAndPath, const int idPath)
{
  ...
  try
  {
    ...
    strSQL = PrepareSQL("insert into program (idProgram, idPath, c%02d, playCount, dateAdded) "
                        "values (NULL, %i, '%s', 0, '%s')",
                        PROGRAMDB_ID_PATH, idPath, strFilenameAndPath.c_str(),
...
int CProgramDatabase::SetDetailsForItem(const CFileItem* item)
{
  ...
  try
  {
    ...
    std::string sql =
        "UPDATE program SET " + GetValueString(*item->GetProgramInfoTag(), PROGRAMDB_ID_MIN,
                                               PROGRAMDB_ID_MAX, DbProgramOffsets);
    std::string strParentPath = URIUtils::GetParentPath(item->GetPath());
    sql += PrepareSQL(", c%02d=%I64u", PROGRAMDB_ID_SIZE,
                      CGUIWindowFileManager::CalculateFolderSize(strParentPath));
...
void CProgramDatabase::SetScraperForPath(const std::string& strPath, const ScraperPtr& scraper)
{
  try
  {
    ...
    std::string strSQL = PrepareSQL("UPDATE path SET strScraper='%s' WHERE idPath=%i",
                                    scraper->ID().c_str(), idPath);
    m_pDS->exec(strSQL);
  }
  catch (...)
  {
...
bool CProgramDatabase::SetProgramSettings(const std::string& strFileNameAndPath,
                                          const std::string& strSettings)
{
  ...
  try
  {
    ...
    strSQL = PrepareSQL("UPDATE program SET strSettings='%s' WHERE idProgram=%i",
                        strSettings.c_str(), idProgram);
    m_pDS->exec(strSQL);
    return true;
  }
  catch (...)
...
bool CProgramDatabase::GetProgramSettings(const std::string& strFileNameAndPath,
...
void CProgramDatabase::UpdateLastPlayed(const std::string& strFilenameAndPath)
{
  ...
  try
  {
    ...
    std::string strSQL =
        PrepareSQL("UPDATE program SET lastPlayed='%s', playCount=playCount + 1 WHERE idProgram=%i",
                   CDateTime::GetCurrentDateTime().GetAsDBDateTime().c_str(), idProgram);
    m_pDS->exec(strSQL);
  }
  catch (...)
  {
...
std::string CProgramDatabase::GetXBEPathByTitleId(const std::string& idTitle)
{
  try
  {
    ...
    std::string strSQL = PrepareSQL("select c%02d from program where c%02d='%s'", PROGRAMDB_ID_PATH,
                                    PROGRAMDB_ID_UNIQUE_ID, idTitle.c_str());
    m_pDS->query(strSQL);
    if (!m_pDS->eof())
...
void CProgramDatabase::GetDetailsForItem(boost::movelib::unique_ptr<dbiplus::Dataset>& pDS,
                                         CFileItem* pItem)
{
  ...
  details.m_lastPlayed.SetFromDBDateTime(
      record->at(PROGRAMDB_DETAILS_PROGRAM_LASTPLAYED).get_asString());
  details.m_dateAdded.SetFromDBDateTime(
      record->at(PROGRAMDB_DETAILS_PROGRAM_DATEADDED).get_asString());
...
{
  std::string strSQL =
      PrepareSQL("SELECT * FROM program WHERE c%02d='game' ORDER BY lastPlayed desc LIMIT 25",
                 PROGRAMDB_ID_TYPE);
  try
  {
    if (NULL == m_pDB.get())
