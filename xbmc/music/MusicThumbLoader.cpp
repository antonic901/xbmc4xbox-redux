...
bool CMusicThumbLoader::FillLibraryArt(CFileItem &item)
{
  ...
  */
    ...
    if (!song.artistCredits.empty())
    {
      ...
      for (VECARTISTCREDITS::const_iterator artistCredit = song.artistCredits.begin(); artistCredit != song.artistCredits.end(); ++artistCredit)
      {
        ...
        if (idArtist > 0)
        {
          ...
          {
            for (std::vector<ArtForThumbLoader>::iterator artitem = artistart.begin(); artitem != artistart.end(); ++artitem)
            {
              if (iOrder > 0)
  ...
  if (artfound)
  {
    ...
    for (std::map<std::string, std::string>::const_iterator discart = discartmap.begin(); discart != discartmap.end(); ++discart)
    {
