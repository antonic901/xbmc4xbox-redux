...
{
  return (size == right.size && mime == right.mime);
}
...
void CMusicInfoTag::SetArtist(const CArtist& artist)
{
  ...
  vecIDs.push_back(artist.strMusicBrainzArtistID);
...
void CMusicInfoTag::Serialize(CVariant& value) const
{
  ...
  if (m_artist.empty())
    value["artist"] =
        StringUtils::Split(GetArtistString(), g_advancedSettings.m_musicItemSeparator);
  ...
  for (VECMUSICROLES::const_iterator it = m_musicRoles.begin(); it != m_musicRoles.end(); ++it)
  {
    const CMusicRole& role = *it;
...
void CMusicInfoTag::Archive(CArchive& ar)
{
  if (ar.IsStoring())
  {
    ...
    for (VECMUSICROLES::const_iterator credit = m_musicRoles.begin(); credit != m_musicRoles.end();
         ++credit)
    {
      ar << credit->GetRoleId();
  ...
  else
  {
    ...
    for (int i = 0; i < iMusicRolesSize; ++i)
    {
      ...
      ar >> strArtist;
      ar >> idArtist;
      CMusicRole role(idRole, strRole, strArtist, idArtist);
      m_musicRoles.push_back(role);
    }
    ar >> m_strMood;
    ar >> m_strRecordLabel;
    ar >> m_Rating;
