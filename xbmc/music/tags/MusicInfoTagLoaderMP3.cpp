...
#define BYTES2INT(b1, b2, b3, b4) \
  (((b1 & 0xFF) << (3 * 8)) | ((b2 & 0xFF) << (2 * 8)) | ((b3 & 0xFF) << (1 * 8)) | \
   ((b4 & 0xFF) << (0 * 8)))

#define UNSYNC(b1, b2, b3, b4) \
  (((b1 & 0x7F) << (3 * 7)) | ((b2 & 0x7F) << (2 * 7)) | ((b3 & 0x7F) << (1 * 7)) | \
   ((b4 & 0x7F) << (0 * 7)))
...
bool CMusicInfoTagLoaderMP3::Load(const CStdString& strFileName,
                                  CMusicInfoTag& tag,
                                  EmbeddedArt* art)
{
  try
...
int CMusicInfoTagLoaderMP3::IsMp3FrameHeader(unsigned long head)
{
  ...
  int framesize =
      tabsel_123[1 - ((head >> 19) & 1)][(4 - ((head >> 17) & 3)) - 1][((head >> 12) & 0xf)] *
          144000 / (freqs[srate] << (1 - ((head >> 19) & 1))) +
      ((head >> 9) & 0x1);
  return framesize;
}
...
#define ID3V2HEADERSIZE 10
  ...
  for (int i = 0; i < iScanSize; i++)
  {
    ...
    for (numFramesCheck = 0; (numFramesCheck < CHECKNUMFRAMES) && framesize; numFramesCheck++)
    {
      unsigned long mpegheader =
          (unsigned long)(((buffer[j] & 255) << 24) | ((buffer[j + 1] & 255) << 16) |
                          ((buffer[j + 2] & 255) << 8) | ((buffer[j + 3] & 255)));
  ...
  {
    unsigned long mpegheader =
        (unsigned long)(((buffer[i] & 255) << 24) | ((buffer[i + 1] & 255) << 16) |
                        ((buffer[i + 2] & 255) << 8) | ((buffer[i + 3] & 255)));
    ...
    if (buffer[i] == 'X' && buffer[i + 1] == 'i' && buffer[i + 2] == 'n' && buffer[i + 3] == 'g')
    {
      ...
      {
        frame_count =
            BYTES2INT(buffer[i + 8], buffer[i + 8 + 1], buffer[i + 8 + 2], buffer[i + 8 + 3]);
        if (buffer[i + 7] & VBR_TOC_FLAG)
        {
          int iOffset = i + 12;
          if (buffer[i + 7] & VBR_BYTES_FLAG)
          {
            nMp3DataSize = BYTES2INT(buffer[i + 12], buffer[i + 12 + 1], buffer[i + 12 + 2],
                                     buffer[i + 12 + 3]);
            iOffset += 4;
          }
      ...
      */
          ...
          (xing[0] == 'I' && xing[1] == 'n' && xing[2] == 'f' && xing[3] == 'o'))
      {
        if (ReadLAMETagInfo(xing - 0x24))
        {
          ...
          __int64 lastSample = (__int64)frame_count * (__int64)tpfbs[layer] -
                               m_seekInfo.GetFirstSample() - m_seekInfo.GetLastSample();
          m_seekInfo.SetDuration((float)lastSample / frequency);
        }
      }
      if (vbri[0] == 'V' && vbri[1] == 'B' && vbri[2] == 'R' && vbri[3] == 'I')
      {
        frame_count = BYTES2INT(vbri[14], vbri[14 + 1], vbri[14 + 2], vbri[14 + 3]);
