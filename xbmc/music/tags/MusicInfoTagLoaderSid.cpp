...
bool CMusicInfoTagLoaderSid::Load(const CStdString& strFileName,
                                  CMusicInfoTag& tag,
                                  EmbeddedArt* art)
{
  ...
  int iHVSC = strFileNameLower.find(
      "hvsc"); // need hvsc in path name since our lookupfile is based on hvsc paths
  if (iHVSC < 0)
  {
  ...
  ifstream f(CSpecialProtocol::TranslatePath(
                 URIUtils::AddFileToFolder(CProfilesManager::Get().GetDatabaseFolder(), "stil.txt"))
                 .c_str()); // changeme?
  if (!f.good())
  {
  ...
  ifstream f2(
      CSpecialProtocol::TranslatePath(
          URIUtils::AddFileToFolder(CProfilesManager::Get().GetDatabaseFolder(), "sidlist.csv"))
          .c_str()); // changeme?
  if (!f2.good())
  {
