...
#define PACK_UINT64(a, b, c, d, e, f, g, h) \
  ((((uint64_t)PACK_UINT32(a, b, c, d)) << 32) | (((uint64_t)PACK_UINT32(e, f, g, h))))
...
int CMusicInfoTagLoaderAAC::ReadADIFDuration(XFILE::CFile& file, int offset)
{
  ...
  if (file.Read(buf, 17) == 17)
  {
    ...
    {
      skip = (buf[4] & 0x80) ? 9 : 0;
      bitrate |= (((uint32_t)buf[4 + skip] & 0x0F) << 19) | ((uint32_t)buf[5 + skip] << 11) |
                 ((uint32_t)buf[6 + skip] << 3) | (((uint32_t)buf[7 + skip] & 0xE0) >> 5);
...
int CMusicInfoTagLoaderAAC::ReadMP4Duration(XFILE::CFile& file,
                                            int64_t position,
                                            int64_t endPosition)
{
  uint8_t buf[8] = {};
  ...
  while ((file.Read(buf, 8) == 8) && (endPosition ? position < endPosition : 1))
  {
    ...
    if (atom_size == 1)
    {
      ...
      {
        atom_size =
            PACK_UINT64(buf2[0], buf2[1], buf2[2], buf2[3], buf2[4], buf2[5], buf2[6], buf2[7]);
      }
      else
        return 0;
    }
