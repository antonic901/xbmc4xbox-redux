...
bool GetEmbeddedThumb(const std::string& path, EmbeddedArt& art)
{
  ...
  boost::movelib::unique_ptr<IMusicInfoTagLoader> loader(CMusicInfoTagLoaderFactory::CreateLoader(item));
  CMusicInfoTag tag;
  if (loader)
...
boost::movelib::unique_ptr<CTexture> CMusicEmbeddedImageFileLoader::Load(const std::string& specialType,
