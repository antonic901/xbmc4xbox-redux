...
void CGUIWindowWeather::UpdateButtons()
{
  ...
  SET_CONTROL_LABEL(WEATHER_LABEL_LOCATION,
                    g_weatherManager.GetLocation(g_weatherManager.GetArea()));
  SET_CONTROL_LABEL(CONTROL_LABELUPDATED, g_weatherManager.GetLastUpdateTime());

  SET_CONTROL_LABEL(WEATHER_LABEL_CURRENT_COND,
                    g_weatherManager.GetInfo(WEATHER_LABEL_CURRENT_COND));
  SET_CONTROL_LABEL(WEATHER_LABEL_CURRENT_TEMP,
                    g_weatherManager.GetInfo(WEATHER_LABEL_CURRENT_TEMP) +
                        g_langInfo.GetTemperatureUnitString());
  SET_CONTROL_LABEL(WEATHER_LABEL_CURRENT_FEEL,
                    g_weatherManager.GetInfo(WEATHER_LABEL_CURRENT_FEEL) +
                        g_langInfo.GetTemperatureUnitString());
  SET_CONTROL_LABEL(WEATHER_LABEL_CURRENT_UVID,
                    g_weatherManager.GetInfo(WEATHER_LABEL_CURRENT_UVID));
  SET_CONTROL_LABEL(WEATHER_LABEL_CURRENT_WIND,
                    g_weatherManager.GetInfo(WEATHER_LABEL_CURRENT_WIND));
  SET_CONTROL_LABEL(WEATHER_LABEL_CURRENT_DEWP,
                    g_weatherManager.GetInfo(WEATHER_LABEL_CURRENT_DEWP) +
                        g_langInfo.GetTemperatureUnitString());
  SET_CONTROL_LABEL(WEATHER_LABEL_CURRENT_HUMI,
                    g_weatherManager.GetInfo(WEATHER_LABEL_CURRENT_HUMI));
  SET_CONTROL_FILENAME(WEATHER_IMAGE_CURRENT_ICON,
                       g_weatherManager.GetInfo(WEATHER_IMAGE_CURRENT_ICON));
  ...
  for (int i = 0; i < NUM_DAYS; i++)
  {
    ...
    SET_CONTROL_LABEL(CONTROL_LABELD0HI + (i * 10), g_weatherManager.GetForecast(i).m_high +
                                                        g_langInfo.GetTemperatureUnitString());
    SET_CONTROL_LABEL(CONTROL_LABELD0LOW + (i * 10), g_weatherManager.GetForecast(i).m_low +
                                                         g_langInfo.GetTemperatureUnitString());
...
void CGUIWindowWeather::SetProperties()
{
  ...
  std::string fanartcode =
      URIUtils::GetFileName(g_weatherManager.GetInfo(WEATHER_IMAGE_CURRENT_ICON));
