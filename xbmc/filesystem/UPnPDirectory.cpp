...
bool CUPnPDirectory::GetResource(const CURL& path, CFileItem& item)
{
  ...
  if (NPT_FAILED(NPT_ContainerFind((*entry)->m_Resources, CProtocolFinder("xbmc-get"), resource)))
  {
    if ((*entry)->m_Resources.GetItemCount())
      resource = (*entry)->m_Resources[0];
    else
    {
      CLog::Log(LOGERROR, "CUPnPDirectory::GetResource - no resources returned for object %s",
                object.c_str());
      return false;
    }
  }
