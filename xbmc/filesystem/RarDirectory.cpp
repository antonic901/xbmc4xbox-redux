...
bool CRarDirectory::GetDirectory(const CURL& urlOrig, CFileItemList& items)
{
  ...
  if (g_RarManager.GetFilesInRar(items, strArchive, true, strPathInArchive))
  {
    ...
    for (int iEntry = 0; iEntry < items.Size(); ++iEntry)
    {
      if (items[iEntry]->IsParentFolder())
        continue;
      items[iEntry]->SetPath(
          URIUtils::AddFileToFolder(strSlashPath, items[iEntry]->GetPath() + strOptions));
  ...
  {
    CLog::Log(LOGWARNING, "%s: rar lib returned no files in archive %s, likely corrupt",
              __FUNCTION__, strArchive.c_str());
    return (false);
  }
}
