...
*/
#ifdef _XBOX
uint32_t DFS_GetPtnStart(uint32_t unit,
...
#endif
{
  ...
  if (mbr->sig_55 != 0x55 || mbr->sig_aa != 0xaa)
    return DFS_ERRMISC;
#endif
  result = (uint32_t)mbr->ptable[pnum].start_0 | (((uint32_t)mbr->ptable[pnum].start_1) << 8) |
  ...
  if (psize)
    *psize = (uint32_t)mbr->ptable[pnum].size_0 | (((uint32_t)mbr->ptable[pnum].size_1) << 8) |
...
*/
#ifdef _XBOX
uint32_t DFS_GetVolInfo(uint32_t unit,
                        uint8_t* scratchsector,
...
#endif
{
  ...
  volinfo->reservedsecs = (uint16_t)lbr->bpb.reserved_l | (((uint16_t)lbr->bpb.reserved_h) << 8);

  volinfo->numsecs = (uint16_t)lbr->bpb.sectors_s_l | (((uint16_t)lbr->bpb.sectors_s_h) << 8);

  if (!volinfo->numsecs)
    volinfo->numsecs = (uint32_t)lbr->bpb.sectors_l_0 | (((uint32_t)lbr->bpb.sectors_l_1) << 8) |
  ...
  volinfo->secperfat = (uint16_t)lbr->bpb.secperfat_l | (((uint16_t)lbr->bpb.secperfat_h) << 8);
  ...
  volinfo->rootentries =
      (uint16_t)lbr->bpb.rootentries_l | (((uint16_t)lbr->bpb.rootentries_h) << 8);
  ...
  if (volinfo->rootentries)
  {
    ...
    volinfo->dataarea =
        volinfo->rootdir +
        (((volinfo->rootentries * 32) + (volinfo->sectorsize - 1)) / volinfo->sectorsize);
  }
  else
  {
    volinfo->dataarea = volinfo->fat1 + (volinfo->secperfat * 2);
    volinfo->rootdir =
        (uint32_t)lbr->ebpb.ebpb32.root_0 | (((uint32_t)lbr->ebpb.ebpb32.root_1) << 8) |
        (((uint32_t)lbr->ebpb.ebpb32.root_2) << 16) | (((uint32_t)lbr->ebpb.ebpb32.root_3) << 24);
  }
...
uint32_t DFS_GetFAT(PVOLINFO volinfo, uint8_t* scratch, uint32_t* scratchcache, uint32_t cluster)
{
  ...
  if (volinfo->filesystem == FAT12)
  {
    ...
    {
      result = (uint32_t)scratch[offset] | ((uint32_t)scratch[offset + 1]) << 8;
    }
    if (cluster & 1)
      result = result >> 4;
    else
  ...
  {
    result = ((uint32_t)scratch[offset] | ((uint32_t)scratch[offset + 1]) << 8 |
              ((uint32_t)scratch[offset + 2]) << 16 | ((uint32_t)scratch[offset + 3]) << 24) &
             0x0fffffff;
  }
  else
...
uint32_t DFS_OpenDir(PVOLINFO volinfo, uint8_t* dirname, PDIRINFO dirinfo)
{
  ...
  if (!strlen((char*)dirname) || (strlen((char*)dirname) == 1 && dirname[0] == DIR_SEPARATOR))
  {
    if (volinfo->filesystem == FAT32)
    {
      ...
      return DFS_ReadSector(volinfo->unit, dirinfo->scratch,
                            volinfo->dataarea + ((volinfo->rootdir - 2) * volinfo->secperclus), 1);
    }
    else
    {
  ...
  else
  {
    ...
    if (volinfo->filesystem == FAT32)
    {
      ...
      // read first sector of directory
      if (DFS_ReadSector(volinfo->unit, dirinfo->scratch,
                         volinfo->dataarea + ((volinfo->rootdir - 2) * volinfo->secperclus), 1))
        return DFS_ERRMISC;
    }
    else
    ...
    while (*ptr)
    {
      ...
      if (!memcmp(de.name, tmpfn, 11) && ((de.attr & ATTR_DIRECTORY) == ATTR_DIRECTORY))
      {
        if (volinfo->filesystem == FAT32)
        {
          dirinfo->currentcluster = (uint32_t)de.startclus_l_l | ((uint32_t)de.startclus_l_h) << 8 |
        ...
        if (DFS_ReadSector(
                ...
                volinfo->dataarea + ((dirinfo->currentcluster - 2) * volinfo->secperclus), 1))
          return DFS_ERRMISC;
      }
...
uint32_t DFS_GetNext(PVOLINFO volinfo, PDIRINFO dirinfo, PDIRENT dirent)
{
  ...
  if (dirinfo->currententry >= volinfo->sectorsize / sizeof(DIRENT))
  {
    ...
    if (dirinfo->currentcluster == 0)
    {
      ...
      if (DFS_ReadSector(volinfo->unit, dirinfo->scratch, volinfo->rootdir + dirinfo->currentsector,
                         1))
        return DFS_ERRMISC;
    }
    ...
    else
    {
      ...
      if (DFS_ReadSector(volinfo->unit, dirinfo->scratch,
                         volinfo->dataarea + ((dirinfo->currentcluster - 2) * volinfo->secperclus) +
                             dirinfo->currentsector,
                         1))
        return DFS_ERRMISC;
    }
...
uint32_t DFS_GetFreeDirEnt(PVOLINFO volinfo, uint8_t* path, PDIRINFO di, PDIRENT de)
{
  ...
  do
  {
    ...
    else if (tempclus == DFS_ALLOCNEW)
    {
      ...
      {
        if (DFS_WriteSector(volinfo->unit, di->scratch,
                            volinfo->dataarea + ((tempclus - 2) * volinfo->secperclus) + i, 1))
          return DFS_ERRMISC;
      }
...
*/
uint32_t DFS_OpenFile(
    PVOLINFO volinfo, uint8_t* path, uint8_t mode, uint8_t* scratch, PFILEINFO fileinfo)
{
  ...
  while (!DFS_GetNext(volinfo, &di, &de))
  {
    if (!memcmp(de.name, filename, 11))
    {
      ...
      if (di.currentcluster == 0)
        ...
        fileinfo->dirsector =
            volinfo->dataarea + ((di.currentcluster - 2) * volinfo->secperclus) + di.currentsector;
      fileinfo->diroffset = di.currententry - 1;
      if (volinfo->filesystem == FAT32)
      {
        fileinfo->cluster = (uint32_t)de.startclus_l_l | ((uint32_t)de.startclus_l_h) << 8 |
                            ((uint32_t)de.startclus_h_l) << 16 | ((uint32_t)de.startclus_h_h) << 24;
      }
      else
      {
        fileinfo->cluster = (uint32_t)de.startclus_l_l | ((uint32_t)de.startclus_l_h) << 8;
      ...
      fileinfo->filelen = (uint32_t)de.filesize_0 | ((uint32_t)de.filesize_1) << 8 |
                          ((uint32_t)de.filesize_2) << 16 | ((uint32_t)de.filesize_3) << 24;
...
*/
uint32_t DFS_ReadFile(
    PFILEINFO fileinfo, uint8_t* scratch, uint8_t* buffer, uint32_t* successcount, uint32_t len)
{
  uint32_t remain;
  ...
  *successcount = 0;
  ...
  while (remain && result == DFS_OK)
  {
    ...
    sector =
        fileinfo->volinfo->dataarea + ((fileinfo->cluster - 2) * fileinfo->volinfo->secperclus) +
        div(div(fileinfo->pointer, fileinfo->volinfo->secperclus * fileinfo->volinfo->sectorsize)
                .rem,
            fileinfo->volinfo->sectorsize)
            .quot;
    ...
    if (div(fileinfo->pointer, fileinfo->volinfo->sectorsize).rem)
    {
      ...
      // just read.
      tempreadsize = fileinfo->volinfo->sectorsize -
                     (div(fileinfo->pointer, fileinfo->volinfo->sectorsize).rem);
...
void DFS_Seek(PFILEINFO fileinfo, uint32_t offset, uint8_t* scratch)
{
  ...
  if (div(fileinfo->pointer, fileinfo->volinfo->secperclus * fileinfo->volinfo->sectorsize).quot ==
      div(fileinfo->pointer + offset, fileinfo->volinfo->secperclus * fileinfo->volinfo->sectorsize)
          .quot)
  {
  ...
  else
  {
    ...
    // seek by clusters
    while (div(fileinfo->pointer, fileinfo->volinfo->secperclus * fileinfo->volinfo->sectorsize)
               .quot !=
