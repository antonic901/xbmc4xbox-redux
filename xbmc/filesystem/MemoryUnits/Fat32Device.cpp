...
void CFat32Device::LogInfo()
{
  ...
  CLog::Log(LOGDEBUG,
            __FUNCTION__ " %d sector/s per cluster, %d reserved sector/s, volume total %d sectors.",
            m_volume.secperclus, m_volume.reservedsecs, m_volume.numsecs);
  CLog::Log(LOGDEBUG, __FUNCTION__ " %d sectors per FAT, first FAT at sector #%d, root dir at #%d.",
            m_volume.secperfat, m_volume.fat1, m_volume.rootdir);
  CLog::Log(LOGDEBUG, __FUNCTION__ " (For FAT32, the root dir is a CLUSTER number, FAT12/16 it is "
                                   "a SECTOR number)");
  CLog::Log(LOGDEBUG, __FUNCTION__ " %d root dir entries, data area commences at sector #%d.",
            m_volume.rootentries, m_volume.dataarea);
  CLog::Log(LOGDEBUG, __FUNCTION__ " %d clusters (%d bytes) in data area, filesystem IDd as %s",
            m_volume.numclusters, m_volume.numclusters * m_volume.secperclus * m_sectorsize,
            GetFileSystem());
}
...
bool CFat32Device::ReadPage(unsigned long page, unsigned char* buffer)
{
  ...
  NTSTATUS status = IoSynchronousFsdRequest(IRP_MJ_READ, (PDEVICE_OBJECT)m_device, buffer,
                                            FAT_PAGE_SIZE, &StartingOffset);
...
bool CFat32Device::WritePage(unsigned long page, unsigned char* buffer)
{
  LARGE_INTEGER StartingOffset;
  StartingOffset.QuadPart = (__int64)page * m_sectorsize;
  NTSTATUS status = IoSynchronousFsdRequest(IRP_MJ_WRITE, (PDEVICE_OBJECT)m_device, buffer,
                                            FAT_PAGE_SIZE, &StartingOffset);
  if (NT_SUCCESS(status))
    return true;
...
unsigned long CFat32Device::ReadSector(unsigned char* buffer,
                                       ...
                                       unsigned long count)
{
  ...
  CLog::Log(LOGERROR, __FUNCTION__ " Error reading sector %u from fat (port %u, slot %u)", sector,
            m_port, m_slot);
  return 1;
}

unsigned long CFat32Device::WriteSector(unsigned char* buffer,
                                        unsigned long sector,
                                        unsigned long count)
{
#ifdef FAT32_ALLOW_WRITING
  ...
  CLog::Log(LOGERROR, __FUNCTION__ " Error writing sector %u from fat (port %u, slot %u)", sector,
            m_port, m_slot);
#endif
  return 1;
}
...
bool CFat32Device::Mount(const char* device)
{
  ...
  NTSTATUS Status = IoSynchronousDeviceIoControlRequest(
      IOCTL_DISK_GET_DRIVE_GEOMETRY, (PDEVICE_OBJECT)m_device, NULL, 0, &DiskGeometry,
  ...
  Status = IoSynchronousDeviceIoControlRequest(
      IOCTL_DISK_GET_PARTITION_INFO, (PDEVICE_OBJECT)m_device, NULL, 0, &PartitionInformation,
  ...
  if (ret)
  {
    ...
    if (m_sectorsize <= 4096)
      CLog::Log(LOGDEBUG,
                __FUNCTION__ " Partition 0 start sector 0x%-08.8lX active %-02.2hX type %-02.2hX "
                             "size %-08.8lX\n",
