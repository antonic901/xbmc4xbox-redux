...
{
public:
  CSector(unsigned char* buffer, unsigned long time)
  {
    fast_memcpy(m_buffer, buffer, FAT_PAGE_SIZE);
    m_usage = 1;
    m_dirty = false;
    m_lastUsed = time;
  };
  ...
  void IncrementUsage(unsigned long time)
  {
    m_usage++;
    m_lastUsed = time;
  };
  unsigned long Usage(unsigned long time) const
  {
    if (m_usage)
      return (time - m_lastUsed) / m_usage;
    return (time - m_lastUsed);
  };
