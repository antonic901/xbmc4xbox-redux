...
struct iso_dirtree* iso9660::ReadRecursiveDirFromSector(DWORD sector, const char* path)
{
  ...
  BOOL bResult = ::ReadFile(m_info.ISO_HANDLE, pCurr_dir_cache, m_info.iso.wSectorSizeLE,
  ...
  if (curr_dir.dwFileLengthLE > m_info.iso.wSectorSizeLE)
  {
    ...
    ::SetFilePointer(m_info.ISO_HANDLE, m_info.iso.wSectorSizeLE * sector, 0, FILE_BEGIN);
    bResult = ::ReadFile(m_info.ISO_HANDLE, pCurr_dir_cache, curr_dir.dwFileLengthLE,
                         &lpNumberOfBytesRead, NULL);
  ...
  while (1)
  {
    ...
    else
    {
      iso9660searchpointer =
          (iso9660searchpointer - (iso9660searchpointer % m_info.iso.wSectorSizeLE)) +
          m_info.iso.wSectorSizeLE;
    }
  ...
  while (1)
  {
    ...
    memcpy(&isodir, pCurr_dir_cache + iso9660searchpointer,
           min(sizeof(isodir), sizeof(m_info.isodir)));
    if (!isodir.ucRecordLength)
      continue;
    if (!(isodir.byFlags & Flag_NotExist))
    {
      if (((isodir.byFlags & Flag_Directory)) && (isodir.Len_Fi > 1))
      {
        ...
        if (bContinue)
        {
...
#ifdef _DEBUG_OUTPUT
          CStdString strTmp;
          strTmp.Format("adding directory sector : %X, File : %s     size = %u     pos = %x\r",
                        sector, temp_text.c_str(), isodir.dwFileLengthLE, isodir.dwFileLocationLE);
          OutputDebugString(strTmp.c_str());
#endif
          ...
          pFile_Pointer->dirpointer =
              ReadRecursiveDirFromSector(isodir.dwFileLocationLE, strPath.c_str());
...
void iso9660::Scan()
{
  ...
  else
  {
    ...
    ::SetFilePointer(m_info.ISO_HANDLE,
                     m_info.iso.wSectorSizeLE *
                         ((iso9660_Directory*)(&m_info.iso.szRootDir))->dwFileLocationLE,
                     0, FILE_BEGIN);
