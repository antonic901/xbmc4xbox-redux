...
public:
  ...
  NPT_XbmcFileStream(NPT_XbmcFileReference file) : m_FileReference(file) {}
...
public:
  // constructors and destructor
  NPT_XbmcFileInputStream(NPT_XbmcFileReference& file) : NPT_XbmcFileStream(file) {}

  // NPT_InputStream methods
  NPT_Result Read(void* buffer, NPT_Size bytes_to_read, NPT_Size* bytes_read);
  NPT_Result Seek(NPT_Position offset) { return NPT_XbmcFileStream::Seek(offset); }
  NPT_Result Tell(NPT_Position& offset) { return NPT_XbmcFileStream::Tell(offset); }
...
public:
  // constructors and destructor
  NPT_XbmcFileOutputStream(NPT_XbmcFileReference& file) : NPT_XbmcFileStream(file) {}

  // NPT_OutputStream methods
  NPT_Result Write(const void* buffer, NPT_Size bytes_to_write, NPT_Size* bytes_written);
  NPT_Result Seek(NPT_Position offset) { return NPT_XbmcFileStream::Seek(offset); }
  NPT_Result Tell(NPT_Position& offset) { return NPT_XbmcFileStream::Tell(offset); }
  NPT_Result Flush() { return NPT_XbmcFileStream::Flush(); }
};
...
+---------------------------------------------------------------------*/
NPT_XbmcFile::NPT_XbmcFile(NPT_File& delegator) : m_Delegator(delegator), m_Mode(0)
{
}
