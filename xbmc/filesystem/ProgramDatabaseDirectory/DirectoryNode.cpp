...
CDirectoryNode* CDirectoryNode::CreateNode(NODE_TYPE Type, const CStdString& strName, CDirectoryNode* pParent)
{
  ...
  case NODE_TYPE_TITLE_APPLICATIONS:
    return new CDirectoryNodeTitleApplications(strName, pParent);
  case NODE_TYPE_GAMES_OVERVIEW:
    return new CDirectoryNodeGamesOverview(strName, pParent);
  case NODE_TYPE_APPLICATIONS_OVERVIEW:
    return new CDirectoryNodeApplicationsOverview(strName, pParent);
...
void CDirectoryNode::AddQueuingFolder(CFileItemList& items) const
{
  ...
  if (g_advancedSettings.m_bProgramLibraryHideAllItems)
    return;
  ...
  if (pItem)
  {
    ...
    pItem->SetSpecialSort(g_advancedSettings.m_bProgramLibraryAllItemsOnBottom ? SortSpecialOnBottom : SortSpecialOnTop);
