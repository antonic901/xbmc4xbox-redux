...
CBuiltins::CommandMap CWeatherBuiltins::GetOperations() const
{
  ...
  CBuiltins::BUILT_IN builtin1 = {"Force weather data refresh", 0, SwitchLocation<0>};
  commands.insert(std::make_pair("weather.refresh", builtin1));

  CBuiltins::BUILT_IN builtin2 = {"Switch to next weather location", 0, SwitchLocation<1>};
  commands.insert(std::make_pair("weather.locationnext", builtin2));
