...
void ControlEdit::setType(int type, const String& heading)
{
  if (pGUIControl)
  {
    XBMCAddonUtils::GuiLock(languageHook, false);
    static_cast<CGUIEditControl*>(pGUIControl)
        ->SetInputType(static_cast<CGUIEditControl::INPUT_TYPE>(type), heading);
  }
}
    ...
    const std::vector<Alternative<String, const XBMCAddon::xbmcgui::ListItem*> >& items)
{
  ...
  for (std::vector<Alternative<String, const XBMCAddon::xbmcgui::ListItem*> >::const_iterator iter =
           items.begin();
       iter != items.end(); ++iter)
    addItem(*iter, false);
