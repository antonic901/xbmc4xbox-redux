    ...
    const std::vector<Alternative<String, const XBMCAddon::xbmcgui::ListItem*> >& items)
{
  XBMC_TRACE;
  XBMCAddonUtils::GuiLock lock(languageHook, false);
  for (std::vector<Alternative<String, const XBMCAddon::xbmcgui::ListItem*> >::const_iterator it =
           items.begin();
       it != items.end(); ++it)
  {
    ...
    case GUI_MSG_CLICKED:
    {
      ...
      if (iControl && iControl != interceptor->GetID()) // pCallbackWindow &&  != this->GetID())
      {
        ...
        //   and we try to do GetControlType on it. So check to make sure it exists
        {
          ...
          {
            invokeCallback(
                new CallbackFunction<WindowXML, int>(this, &WindowXML::onClick, iControl));
            PulseActionEvent();
            return true;
          }
          else if (controlClicked->IsContainer() &&
                   message.GetParam1() == ACTION_MOUSE_DOUBLE_CLICK)
          {
            invokeCallback(
