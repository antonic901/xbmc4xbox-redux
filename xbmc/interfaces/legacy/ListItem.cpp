...
void ListItem::setProperty(const char* key, const String& value)
{
  ...
  { // special case for start offset - don't actually store in a property,
    ...
    item->m_lStartOffset =
        (int)(atof(value.c_str()) * 75.0); // we store the offset in frames, or 1/75th of a second
  }
  ...
  if (strcmpi(type, "video") == 0)
  {
    ...
    for (InfoLabelDict::const_iterator it = infoLabels.begin(); it != infoLabels.end(); ++it)
    {
      ...
      else if (key == "cast" || key == "castandrole")
      {
        ...
        for (std::vector<InfoLabelStringOrTuple>::const_iterator viter = listValue.begin();
             viter != listValue.end(); ++viter)
        {
    ...
    for (InfoLabelDict::const_iterator it = infoLabels.begin(); it != infoLabels.end(); ++it)
    {
      ...
      if (key == "dbid" && !type.empty())
        ...
        musictag.SetMusicBrainzArtistID(
            StringUtils::Split(value, g_advancedSettings.m_musicItemSeparator));
...
void ListItem::setCast(const std::vector<Properties>& actors)
{
  ...
  for (std::vector<Properties>::const_iterator dictionary = actors.begin();
       dictionary != actors.end(); ++dictionary)
  {
    SActorInfo info;
...
void ListItem::setAvailableFanart(const std::vector<Properties>& images)
{
  ...
  for (std::vector<Properties>::const_iterator dictionary = images.begin();
       dictionary != images.end(); ++dictionary)
  {
    std::string image;
    std::string preview;
    std::string colors;
    for (XBMCAddon::Properties::const_iterator it = (*dictionary).begin();
                                                  ...
                                                  std::string value)
{
  ...
  for (std::vector<XBMCAddon::xbmcgui::InfoLabelStringOrTuple>::const_iterator el =
           vecInfoLabel.begin();
       el != vecInfoLabel.end(); ++el)
  {
    if ((*el).which() == second)
