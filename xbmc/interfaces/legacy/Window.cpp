...
public:
  inline explicit MaybeLock(CCriticalSection* p_lock) : lock(p_lock)
  {
    if (lock)
      lock->lock();
  }
...
public:
  inline SingleLockWithDelayGuard(CCriticalSection& ccrit, LanguageHook* lh) : dcg(lh), lock(ccrit)
  {
    lock.lock();
  }
...
public:
  inline ProxyExistingWindowInterceptor(CGUIWindow* window) : cguiwindow(window) { XBMC_TRACE; }
...
int Window::getNextAvailableWindowId()
{
  ...
  while (id < WINDOW_PYTHON_END && g_windowManager.GetWindow(id) != NULL)
    id++;
  return id;
}
