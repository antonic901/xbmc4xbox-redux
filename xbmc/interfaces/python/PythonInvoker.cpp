...
std::vector<wchar_t> stringToCharVector(const std::wstring& s) {
    return std::vector<wchar_t>(s.c_str(), s.c_str() + s.length() + 1);
}
...
wchar_t* charVectorToPointer(std::vector<wchar_t>& v) {
    return &v[0];
}
