...
CScriptInvocationManager::CScriptInvocationManager() : m_nextId(0)
{
}
...
void CScriptInvocationManager::Process()
{
  ...
  for (std::vector<LanguageInvokerThread>::const_iterator it = tempList.begin();
       it != tempList.end(); ++it)
  ...
  for (LanguageInvocationHandlerMap::iterator it = m_invocationHandlers.begin();
       it != m_invocationHandlers.end(); ++it)
    it->second->Process();
}
...
void CScriptInvocationManager::Uninitialize()
{
  ...
  m_lastInvokerThread = CLanguageInvokerThreadPtr();

  // make sure all scripts are done
  std::vector<LanguageInvokerThread> tempList;
  for (LanguageInvokerThreadMap::iterator script = m_scripts.begin(); script != m_scripts.end();
       ++script)
  ...
  for (std::vector<LanguageInvokerThread>::iterator it = tempList.begin(); it != tempList.end();
       ++it)
  {
    if (!it->done)
      it->thread->Stop(true);
    ...
    ILanguageInvocationHandler* invocationHandler, const std::string& extension)
{
  ...
  bool known = false;
  for (std::map<std::string, ILanguageInvocationHandler*>::const_iterator it =
    ...
    ILanguageInvocationHandler* invocationHandler, const std::set<std::string>& extensions)
{
  ...
  for (std::set<std::string>::const_iterator extension = extensions.begin();
