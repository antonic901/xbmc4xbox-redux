...
protected:
  ...
  virtual ~CRunningScriptsHandler() {}
  ...
  bool RunScript(TScript* script,
                 ...
                 bool resume)
  {
    if (script == NULL || addon == NULL || path.empty())
      return false;
    ...
    // run the script
    bool result = CScriptRunner::RunScript(addon, path, handle, resume);
  ...
  {
    CSingleLock lock(s_critical);
    typename std::map<typename CRunningScriptsHandler<TScript>::HandleType, TScript*>::iterator scriptHandle = s_scriptHandles.find(handle);
