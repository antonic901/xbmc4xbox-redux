...
{
  const char** pointer;
  charPtrPtrAdapter(const char** p) : pointer(p) {}
...
void CCharsetConverter::SettingOptionsCharsetsFiller(
    ...
    void* data)
{
  ...
  for (int i = 0; i < (int)vecCharsets.size(); ++i)
    list.push_back(
        make_pair(vecCharsets[i], g_charsetConverter.getCharsetNameByLabel(vecCharsets[i])));
}