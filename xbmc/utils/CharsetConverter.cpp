...
#if defined(FRIBIDI_CHAR_SET_NOT_FOUND) || defined(_XBOX)
...
{
  stringDst.clear();
  vector<std::string> lines = StringUtils2::Split(stringSrc, "\n");
...
bool CCharsetConverter::isBidiCharset(const std::string& charset)
{
  ...
  {
    if (StringUtils2::EqualsNoCase(charset, c->charset))
      return true;
  }
  return false;
}
...
void CCharsetConverter::reset(void)
{
  ...
  for(SFribidMapping* c = g_fribidi; c->charset; c++)
  {
    if (StringUtils2::EqualsNoCase(strCharset, c->charset))
    {
