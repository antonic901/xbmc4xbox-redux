...
extern "C"
{
  XBOXAPI NTSTATUS WINAPI MU_CreateDeviceObject(IN ULONG Port,
                                                IN ULONG Slot,
                                                IN POBJECT_STRING DeviceName);

  XBOXAPI VOID WINAPI MU_CloseDeviceObject(IN ULONG Port, IN ULONG Slot);

  XBOXAPI PDEVICE_OBJECT WINAPI MU_GetExistingDeviceObject(IN ULONG Port, IN ULONG Slot);
};
...
bool CMemoryUnitManager::Update()
{
  ...
  CLog::Log(LOGDEBUG, __FUNCTION__ " Reports added device %08x and removed device %08x", newdev,
            deaddev);
...
bool CMemoryUnitManager::UnMountDevice(unsigned long port, unsigned long slot)
{
  ...
  for (vector<IDevice*>::iterator it = m_memUnits.begin(); it != m_memUnits.end(); ++it)
  {
    ...
    {
      CLog::Log(LOGDEBUG, __FUNCTION__ " Found MU device to unmount (port %i, slot %i)", port,
                slot);
      device->UnMount();
...
IDevice* CMemoryUnitManager::GetDevice(unsigned char unit) const
{
  ...
  {
    CLog::Log(LOGERROR, __FUNCTION__ " Attempt to access memory device %i when it doesn't exist",
              unit);
    return NULL;
  }
...
void CMemoryUnitManager::GetMemoryUnitSources(VECSOURCES& shares)
{
  for (unsigned int i = 0; i < m_memUnits.size(); i++)
  {
    ...
    if (volumeName.IsEmpty())
      share.strName.Format("%s %i (%s)", g_localizeStrings.Get(20136).c_str(), i + 1,
                           m_memUnits[i]->GetFileSystem());
    else
      share.strName.Format("%s %i (%s) (%s)", g_localizeStrings.Get(20136).c_str(), i + 1,
                           volumeName.c_str(), m_memUnits[i]->GetFileSystem());
