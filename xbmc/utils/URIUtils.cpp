...
void URIUtils::GetCommonPath(std::string& strParent, const std::string& strPath)
{
  ...
  unsigned int j = 1;
  while (j <= min(strParent.size(), strPath.size()) &&
...
{
  return url.IsProtocol("zip") || url.IsProtocol("rar") || url.IsProtocol("apk") ||
         url.IsProtocol("bluray") || url.IsProtocol("udf");
}
...
{
  for (CAdvancedSettings::StringMapping::iterator i =
           g_advancedSettings.m_pathSubstitutions.begin();
...
bool URIUtils::PathEquals(const std::string& url,
...
bool URIUtils::IsFTP(const CStdString& strFile)
{
  ...
  return IsProtocol(strFile2, "ftp") || IsProtocol(strFile2, "ftps");
}
...
bool URIUtils::IsDAV(const CStdString& strFile)
{
  ...
  return IsProtocol(strFile2, "dav") || IsProtocol(strFile2, "davs");
}
...
bool URIUtils::IsLiveTV(const CStdString& strFile)
{
  ...
  if (IsVTP(strFile) || IsProtocol(strFile, "sap") ||
      (StringUtils::EndsWithNoCase(strFileWithoutSlash, ".pvr") &&
       !PathStarts(strFileWithoutSlash, "pvr://recordings")))
    return true;
