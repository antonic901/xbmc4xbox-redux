  ...
  if (!progressTrackingFile.empty())
  {
        ...
        && UPNP::CUPnP::SaveFileState(item, bookmark, updatePlayCount))
    {
      ...
      CGUIMessage message(GUI_MSG_NOTIFY_ALL, 0, 0, GUI_MSG_UPDATE_ITEM, 0,
                          boost::make_shared<CFileItem>(updatedItem));
    ...
    if (item.IsVideo())
    {
      ...
      else
      {
        ...
        if (updateListing)
        {
          ...
          const boost::shared_ptr<const CApplicationStackHelper> stackHelper = components.GetComponent<CApplicationStackHelper>();
