...
bool CGFFPatch::FFPatch(const std::string& m_FFPatchFilePath,
                        std::string& strNEW_FFPatchFilePath) // apply the patch
{
  CFile xbe;
  ...
  while (1)
  {
    ...
    if ((location == 0) && (patchCount == 0))
    {
      ...
      CLog::Log(LOGDEBUG,
                __FUNCTION__ " - Could be a previos patched, homebrew or a non patchable xbe!");
...
BOOL CGFFPatch::Patch2(BYTE* pbuffer, UINT location)
{
  ...
  replaceConditionalJump(&pbuffer[0], tmp,
                         10); // see if there's a 2nd conditional jump we need to worry about.
...
BOOL CGFFPatch::Patch4(BYTE* pbuffer, UINT location)
{
  ...
  replaceConditionalJump(&pbuffer[0], tmp, 0x20);
  pbuffer[tmp] =
      pbuffer[location - 4]; // move the 2 bytes before the pushes start.  Prolly an "xor reg,reg"
...
int CGFFPatch::examinePatch(BYTE* pbuffer, UINT location)
{
  ...
  if (findConditionalJump(&pbuffer[0], tmp, 0x20) == FALSE)
    return 0;
  if ((pbuffer[location - 2] == 0x6a) && (pbuffer[location - 1] >= 0) &&
      (pbuffer[location - 1] < 6) &&
      ((pbuffer[location - 3] == 0) && (pbuffer[location - 4] == 0x6a) ||
       ((pbuffer[location - 3] & 0x50) == 0x50)))
    return 1;
  if (((pbuffer[location - 1] & 0x50) == 0x50) && (pbuffer[location - 2] == 0) &&
      (pbuffer[location - 3] == 0x6a))
    return 2;

  if (((pbuffer[location - 1] & 0x50) == 0x50) && ((pbuffer[location - 2] & 0x50) == 0x50) &&
...
UINT CGFFPatch::searchData(BYTE* pBuffer, UINT startPos, UINT size)
{
  ...
  {
    if ((pBuffer[i] == 0x6a) && (pBuffer[i + 1] == 0x0b) && ((pBuffer[i + 2] & 0x50) == 0x50) &&
        (pBuffer[i + 3] == 0xff))
      return i;
  }
