...
{
  CSingleLock lock(m_obsCritSection);
  std::vector<Observer *>::iterator iter = std::remove(m_observers.begin(), m_observers.end(), obs);
...
void Observable::SendMessage(const ObservableMessage message)
{
  ...
  for (std::vector<Observer *>::iterator observer = m_observers.begin(); observer != m_observers.end(); ++observer)
  {
