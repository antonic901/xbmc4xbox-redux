...
bool CRecentlyAddedJob::UpdateProgram()
{
  ...
  CLog::Log(LOGDEBUG,
            "CRecentlyAddedJob::UpdatePrograms() - Running RecentlyAdded home screen update");
  ...
  if (database.GetRecentlyPlayedGames(items))
  {
    for (int i = 0; i < items.Size(); ++i)
    {
      ...
      window->SetProperty("RecentlyPlayedGame." + value + ".Path",
                          item->GetProgramInfoTag()->m_strFileNameAndPath);
