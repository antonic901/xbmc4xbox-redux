...
const std::string ContentUtils::GetPreferredArtImage(const CFileItem& item)
{
  ...
  {
    std::string preferredArtType = GetPreferredArtType(item.GetVideoInfoTag()->m_type);
...
boost::movelib::unique_ptr<CFileItem> ContentUtils::GeneratePlayableTrailerItem(const CFileItem& item,
                                                                     const std::string& label)
{
  boost::movelib::unique_ptr<CFileItem> trailerItem = boost::movelib::make_unique<CFileItem>();
  ...
  *videoInfoTag = *item.GetVideoInfoTag();
  ...
  videoInfoTag->m_strTitle = StringUtils::Format("%s (%s)", videoInfoTag->m_strTitle.c_str(), label.c_str());
  trailerItem->SetArt(item.GetArt());
  videoInfoTag->m_iDbId = -1;
  videoInfoTag->m_iFileId = -1;
  return boost::move(trailerItem);
}
