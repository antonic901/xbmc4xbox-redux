...
extern const struct LCENTRY g_iso639_1[188];
extern const struct LCENTRY g_iso639_2[540];
...
extern const ISO639 LanguageCodes[192];
                                                 ...
                                                 bool checkWin32Locales /*= false*/)
{
  ...
  for (size_t i = 0; i < sizeof(LanguageCodes) / sizeof(ISO639); ++i)
  {
    if (strISO6391Lower == LanguageCodes[i].iso639_1)
    {
      if (checkWin32Locales && LanguageCodes[i].win_id)
      {
        strISO6392B = LanguageCodes[i].win_id;
        return true;
      }

      strISO6392B = LanguageCodes[i].iso639_2b;
      return true;
    }
  }
                                          ...
                                          bool checkWin32Locales /* = false */)
{
  ...
  if (strCharCode.size() == 3)
  {
    ...
    for (size_t i = 0; i < sizeof(LanguageCodes) / sizeof(ISO639); ++i)
    {
      if (charCode == LanguageCodes[i].iso639_2b ||
          (checkWin32Locales && LanguageCodes[i].win_id != NULL && charCode == LanguageCodes[i].win_id))
      {
    ...
    for (size_t i = 0; i < sizeof(RegionCodes) / sizeof(ISO3166_1); ++i)
    {
      if (charCode == RegionCodes[i].alpha3)
      {
        strISO6392B = charCode;
        return true;
      }
  ...
  {
    for (size_t i = 0; i < sizeof(g_iso639_2) / sizeof(LCENTRY); ++i)
    {
      if (StringUtils::EqualsNoCase(strCharCode, g_iso639_2[i].name))
      {
        strISO6392B = CodeToString(g_iso639_2[i].code);
        return true;
      }
    }
                                          ...
                                          bool checkWin32Locales /* = false */)
{
  ...
  for (size_t i = 0; i < sizeof(LanguageCodes) / sizeof(ISO639); ++i)
  {
    if (strISO6392T == LanguageCodes[i].iso639_2b ||
        (checkWin32Locales && LanguageCodes[i].win_id != NULL && strISO6392T == LanguageCodes[i].win_id))
    {
      if (LanguageCodes[i].iso639_2t != NULL)
        strISO6392T = LanguageCodes[i].iso639_2t;
      return true;
    }
  }
  return false;
                                                              ...
                                                              std::string& strISO31661Alpha3)
{
  ...
  for (size_t i = 0; i < sizeof(RegionCodes) / sizeof(ISO3166_1); ++i)
  {
    if (strLower == RegionCodes[i].alpha2)
    {
      strISO31661Alpha3 = RegionCodes[i].alpha3;
      return true;
    }
  }
    ...
    const std::string& strWindowsLanguageCode, std::string& strISO6392B)
{
  ...
  for (size_t i = 0; i < sizeof(LanguageCodes) / sizeof(ISO639); ++i)
  {
    if ((LanguageCodes[i].win_id && strLower == LanguageCodes[i].win_id) || strLower == LanguageCodes[i].iso639_2b)
    {
      strISO6392B = LanguageCodes[i].iso639_2b;
      return true;
    }
...
bool CLangCodeExpander::ConvertToISO6391(const std::string& lang, std::string& code)
{
  ...
  else if (lang.length() == 3)
  {
    ...
    for (size_t i = 0; i < sizeof(LanguageCodes) / sizeof(ISO639); ++i)
    {
      if (lower == LanguageCodes[i].iso639_2b || (LanguageCodes[i].win_id && lower == LanguageCodes[i].win_id))
      {
        code = LanguageCodes[i].iso639_1;
        return true;
      }
    }
    ...
    for (size_t i = 0; i < sizeof(RegionCodes) / sizeof(ISO3166_1); ++i)
    {
      if (lower == RegionCodes[i].alpha3)
      {
        code = RegionCodes[i].alpha2;
        return true;
      }
    }
  }
...
bool CLangCodeExpander::ReverseLookup(const std::string& desc, std::string& code)
{
  ...
  for (unsigned int i = 0; i < sizeof(g_iso639_1) / sizeof(LCENTRY); ++i)
  {
    ...
    {
      code = CodeToString(g_iso639_1[i].code);
      return true;
    }
  }
  ...
  for (unsigned int i = 0; i < sizeof(g_iso639_2) / sizeof(LCENTRY); ++i)
  {
    ...
    {
      code = CodeToString(g_iso639_2[i].code);
      return true;
    }
  }
    ...
    LANG_LIST list /* = LANG_LIST::DEFAULT */)
{
  ...
  if (format == CLangCodeExpander::ISO_639_2)
  {
    for (size_t i = 0; i < sizeof(g_iso639_2) / sizeof(LCENTRY); ++i)
    {
      langMap.insert(std::make_pair(CodeToString(g_iso639_2[i].code), g_iso639_2[i].name));
    }
  }
  else
  {
    for (size_t i = 0; i < sizeof(g_iso639_1) / sizeof(LCENTRY); ++i)
    {
      langMap.insert(std::make_pair(CodeToString(g_iso639_1[i].code), g_iso639_1[i].name));
    }
  }
  ...
  {
    for (std::map<std::string, std::string>::const_iterator value = m_mapUser.begin(); value != m_mapUser.end(); ++value)
    {
  ...
  for (std::map<std::string, std::string>::const_iterator lang = langMap.begin(); lang != langMap.end(); ++lang)
  {
...
// clang-format off
const ISO639 LanguageCodes[192] = {
    {"aa", "aar", NULL, NULL},
    {"ab", "abk", NULL, NULL},
    {"af", "afr", NULL, NULL},
    {"ak", "aka", NULL, NULL},
