...
#include "addons/addoninfo/AddonInfo.h" // AddonUpdateRule
...
public:
   ...
   * \return true on success, false otherwise
   */
  bool AddUpdateRuleToList(CAddonDatabase& db, const std::string& id, AddonUpdateRule::Type updateRule);
...
private:
   ...
   */
  bool IsUpdateableByRule(const std::string& id, AddonUpdateRule::Type updateRule) const;
  ...
  std::map<std::string, std::vector<AddonUpdateRule::Type> > m_updateRules;
};
