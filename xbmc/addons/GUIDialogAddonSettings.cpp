...
bool CGUIDialogAddonSettings::OnMessage(CGUIMessage& message)
{
  switch (message.GetMessage())
  {
    ...
    case GUI_MSG_FOCUSED:
    {
      ...
      if (focusedControl >= CONTROL_START_SECTION &&
          focusedControl < (int)(CONTROL_START_SECTION + m_totalSections) &&
...
bool CGUIDialogAddonSettings::ShowAndGetInput(const AddonPtr& addon, bool saveToDisk /* = true */)
{
  ...
  if (addon->HasSettings())
  {
    ...
    // Set the heading
    std::string heading = StringUtils::Format("$LOCALIZE[10004] - %s",
                                              addon->Name().c_str()); // "Settings - AddonName"
...
bool CGUIDialogAddonSettings::ShowVirtualKeyboard(int iControl)
{
  ...
  while (setting)
  {
    if (controlId == iControl)
    {
      ...
      if (control && control->GetControlType() == CGUIControl::GUICONTROL_BUTTON && !id.empty() &&
          !type.empty())
      {
        const char* option = setting->Attribute("option");
        ...
        else if (type == "select")
        {
          ...
          if (pDlg)
          {
            ...
            { // localize
              ...
              for (unsigned int i = 0; i < valuesVec.size(); i++)
              {
                ...
                std::string localized =
                    g_localizeStrings.GetAddonString(m_addon->ID(), atoi(valuesVec[i].c_str()));
                if (localized.empty())
            ...
            {
              valuesVec = GetFileEnumValues(source, XMLUtils::GetAttribute(setting, "mask"),
                                            XMLUtils::GetAttribute(setting, "option"));
            }
            ...
            for (unsigned int i = 0; i < valuesVec.size(); i++)
            {
              ...
              if (selected == (int)i ||
                  (selected < 0 && StringUtils::EqualsNoCase(valuesVec[i], value)))
                pDlg->SetSelected(
                    i); // FIXME: the SetSelected() does not select "i", it always defaults to the first position
            }
            pDlg->Open();
        ...
        else if (type == "audio" || type == "video" || type == "image" || type == "executable" ||
                 type == "file" || type == "folder")
        {
          ...
          else
          {
            ...
            if (option)
            {
              ...
              bUseFileDirectories =
                  find(options.begin(), options.end(), "treatasfolder") != options.end();
            }

            if (CGUIDialogFileBrowser::ShowAndGetFile(localShares, strMask, label, value,
                                                      bUseThumbs, bUseFileDirectories))
              ((CGUIButtonControl*)control)->SetLabel2(value);
          }
        }
        else if (type == "date")
        ...
        else if (type == "addon")
        {
          ...
          if (strType)
          {
            ...
            for (std::vector<std::string>::iterator i = addonTypes.begin(); i != addonTypes.end();
                 ++i)
            {
              StringUtils::Trim(*i);
      ...
      switch (control->GetControlType())
      {
        ...
        case CGUIControl::GUICONTROL_SETTINGS_SLIDER:
        {
          ...
          if (option.empty() || StringUtils::EqualsNoCase(option, "float"))
            value =
                StringUtils::Format("%f", ((CGUISettingsSliderControl*)control)->GetFloatValue());
          else
...
void CGUIDialogAddonSettings::SaveSettings(void)
{
  ...
  for (std::map<std::string, std::string>::iterator i = m_settings.begin(); i != m_settings.end();
       ++i)
...
void CGUIDialogAddonSettings::FreeSections()
{
  CGUIControlGroupList* group =
      dynamic_cast<CGUIControlGroupList*>(GetControl(CONTROL_SECTION_AREA));
  if (group)
  {
    group->FreeResources();
    group->ClearAll();
...
void CGUIDialogAddonSettings::FreeControls()
{
  ...
  CGUIControlGroupList* control =
      dynamic_cast<CGUIControlGroupList*>(GetControl(CONTROL_SETTINGS_AREA));
  if (control)
  {
...
void CGUIDialogAddonSettings::CreateSections()
{
  ...
  { // add a category
    ...
    {
      CLog::Log(LOGERROR, "%s - cannot have more than %d categories - simplify your addon!",
                __FUNCTION__, CONTROL_START_SETTING - CONTROL_START_SECTION);
      break;
    }
...
void CGUIDialogAddonSettings::CreateControls()
{
  ...
  CGUISpinControlEx* pOriginalSpin =
      dynamic_cast<CGUISpinControlEx*>(GetControl(CONTROL_DEFAULT_SPIN));
  CGUIRadioButtonControl* pOriginalRadioButton =
      dynamic_cast<CGUIRadioButtonControl*>(GetControl(CONTROL_DEFAULT_RADIOBUTTON));
  CGUIButtonControl* pOriginalButton =
      dynamic_cast<CGUIButtonControl*>(GetControl(CONTROL_DEFAULT_BUTTON));
  CGUIImage* pOriginalImage = dynamic_cast<CGUIImage*>(GetControl(CONTROL_DEFAULT_SEPARATOR));
  CGUILabelControl* pOriginalLabel =
      dynamic_cast<CGUILabelControl*>(GetControl(CONTROL_DEFAULT_LABEL_SEPARATOR));
  CGUISettingsSliderControl* pOriginalSlider =
      dynamic_cast<CGUISettingsSliderControl*>(GetControl(CONTROL_DEFAULT_SLIDER));
  ...
  while (setting)
  {
    ...
    if (!type.empty())
    {
      ...
      if (type == "text" || type == "ipaddress" || type == "number" || type == "video" ||
          type == "audio" || type == "image" || type == "folder" || type == "executable" ||
          type == "file" || type == "action" || type == "date" || type == "time" ||
        ...
        if (!id.empty())
        {
          ...
          else
          {
            ...
            else if (type == "select" && !lvalues.empty())
            {
              ...
              {
                std::string label = g_localizeStrings.GetAddonString(
                    m_addon->ID(), atoi(valuesVec[selected].c_str()));
                if (label.empty())
      ...
      else if (type == "fileenum" && !id.empty())
      {
        ...
        std::vector<std::string> items =
            GetFileEnumValues(values, XMLUtils::GetAttribute(setting, "mask"),
                              XMLUtils::GetAttribute(setting, "option"));
      ...
      else if (type == "rangeofnum" && !id.empty())
      {
        ...
        if (setting->Attribute("valueformat"))
          valueformat = g_localizeStrings.GetAddonString(m_addon->ID(),
                                                         atoi(setting->Attribute("valueformat")));
        for (int i = 0; i < elements; i++)
        {
          ...
          if (elements < 2)
            ...
            valuestring = StringUtils::Format(
                valueformat.c_str(), rangestart + (rangeend - rangestart) / (elements - 1) * i);
      ...
      else if (type == "slider" && !id.empty())
      {
        ...
        std::vector<std::string> range =
            StringUtils::Split(XMLUtils::GetAttribute(setting, "range"), ',');
        if (range.size() > 1)
        {
...
std::vector<std::string> CGUIDialogAddonSettings::GetFileEnumValues(
    const std::string& path, const std::string& mask, const std::string& options) const
{
...
bool CGUIDialogAddonSettings::GetCondition(const std::string& condition, const int controlId)
{
  ...
  for (unsigned int i = 0; i < conditionVec.size(); i++)
  {
    ...
    if (!TranslateSingleString(conditionVec[i], condVec))
      continue;
...
bool CGUIDialogAddonSettings::TranslateSingleString(const std::string& strCondition,
                                                    std::vector<std::string>& condVec)
{
