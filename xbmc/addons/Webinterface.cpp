...
CWebinterface::CWebinterface(const AddonInfoPtr& addonInfo)
  : CAddon(addonInfo, AddonType::WEB_INTERFACE), m_entryPoint(WEBINTERFACE_DEFAULT_ENTRY_POINT), m_type(WebinterfaceTypeStatic)
{
  ...
  if (StringUtils::EqualsNoCase(webinterfaceType, "wsgi"))
    ...
    CLog::Log(LOGWARNING,
              "CWebinterface::%s: Addon \"%s\" has specified an unsupported type \"%s\"", __FUNCTION__,
