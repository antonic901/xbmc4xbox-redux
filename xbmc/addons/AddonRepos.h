...
#include "addons/AddonManager.h" // AddonCheckType
...
{
  boost::shared_ptr<IAddon> m_installed;
  boost::shared_ptr<IAddon> m_update;
};
...
public:
  ...
  explicit CAddonRepos(const boost::shared_ptr<IAddon>& repoAddon); // load add-ons of a specific repo
   ...
   *             or are outdated
   */
  void BuildUpdateOrOutdatedList(const std::vector<boost::shared_ptr<IAddon> >& installed,
   ...
   */
  void BuildAddonsWithUpdateList(const std::vector<boost::shared_ptr<IAddon> >& installed,
   ...
   */
  static bool IsFromOfficialRepo(const boost::shared_ptr<IAddon>& addon,
   ...
   */
  bool DoAddonUpdateCheck(const boost::shared_ptr<IAddon>& addon,
   ...
   */
  void GetLatestAddonVersions(std::vector<boost::shared_ptr<IAddon> >& addonList) const;
   ...
   * \param[out] addonList retrieved addon list in a vector
   */
  void GetLatestAddonVersionsFromAllRepos(std::vector<boost::shared_ptr<IAddon> >& addonList) const;
   ...
   */
  void BuildCompatibleVersionsList(std::vector<boost::shared_ptr<IAddon> >& compatibleVersions) const;
...
private:
   ...
   * \return true on success, false otherwise
   */
  bool LoadAddonsFromDatabase(const std::string& addonId, const boost::shared_ptr<IAddon>& repoAddon);
   ...
   */
  bool FindAddonAndCheckForUpdate(const boost::shared_ptr<IAddon>& addonToCheck,
   ...
   */
  void AddAddonIfLatest(const boost::shared_ptr<IAddon>& addonToAdd,
  ...
  std::vector<boost::shared_ptr<IAddon> > m_allAddons;

  std::map<std::string, boost::shared_ptr<IAddon> > m_latestOfficialVersions;
  std::map<std::string, boost::shared_ptr<IAddon> > m_latestPrivateVersions;
  std::map<std::string, std::map<std::string, boost::shared_ptr<IAddon> > > m_latestVersionsByRepo;
  std::map<std::string, std::multimap<std::string, boost::shared_ptr<IAddon> > > m_addonsByRepoMap;
};
