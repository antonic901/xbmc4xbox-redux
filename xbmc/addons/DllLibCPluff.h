...
public:
  ...
  virtual cp_status_t register_logger(cp_context_t* ctx,
                                      cp_logger_func_t logger,
                                      void* user_data,
                                      cp_log_severity_t min_severity) = 0;
  ...
  virtual cp_plugin_info_t* get_plugin_info(cp_context_t* ctx,
                                            const char* id,
                                            cp_status_t* status) = 0;
  virtual cp_plugin_info_t** get_plugins_info(cp_context_t* ctx, cp_status_t* status, int* num) = 0;
  virtual cp_extension_t** get_extensions_info(cp_context_t* ctx,
                                               const char* extpt_id,
                                               cp_status_t* status,
                                               int* num) = 0;
  ...
  virtual void* resolve_symbol(cp_context_t* ctx,
                               const char* id,
                               const char* name,
                               cp_status_t* status) = 0;
  virtual void release_symbol(cp_context_t* ctx, const void* ptr) = 0;
  virtual cp_plugin_info_t* load_plugin_descriptor(cp_context_t* ctx,
                                                   const char* path,
                                                   cp_status_t* status) = 0;
  virtual cp_plugin_info_t* load_plugin_descriptor_from_memory(cp_context_t* ctx,
                                                               const char* buffer,
...
class DllLibCPluff : public DllDynamic, DllLibCPluffInterface
{
  ...
  DEFINE_METHOD4(cp_status_t,
                 register_logger,
                 (cp_context_t * p1, cp_logger_func_t p2, void* p3, cp_log_severity_t p4))
  ...
  DEFINE_METHOD3(cp_plugin_info_t*,
                 get_plugin_info,
                 (cp_context_t * p1, const char* p2, cp_status_t* p3))
  DEFINE_METHOD3(cp_plugin_info_t**,
                 get_plugins_info,
                 (cp_context_t * p1, cp_status_t* p2, int* p3))
  DEFINE_METHOD4(cp_extension_t**,
                 get_extensions_info,
                 (cp_context_t * p1, const char* p2, cp_status_t* p3, int* p4))
  ...
  DEFINE_METHOD4(void*,
                 resolve_symbol,
                 (cp_context_t * p1, const char* p2, const char* p3, cp_status_t* p4))
  DEFINE_METHOD2(void, release_symbol, (cp_context_t * p1, const void* p2))
  DEFINE_METHOD3(cp_plugin_info_t*,
                 load_plugin_descriptor,
                 (cp_context_t * p1, const char* p2, cp_status_t* p3))
  DEFINE_METHOD4(cp_plugin_info_t*,
                 load_plugin_descriptor_from_memory,
                 (cp_context_t * p1, const char* p2, unsigned int p3, cp_status_t* p4))
