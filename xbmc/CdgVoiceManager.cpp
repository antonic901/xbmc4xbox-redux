...
HRESULT CCdgChatter::Initialize(CCdgVoiceManager* pManager, DWORD dwPort, CDG_DEVICE_TYPE device)
{
  ...
  hr = XVoiceCreateMediaObjectEx(ppDevice, m_dwPort, m_pVoiceManager->m_dwNumPackets, &m_wfx,
  ...
  DSMIXBINVOLUMEPAIR dsmbvp[6] = {
      {DSMIXBIN_FRONT_LEFT, DSBVOLUME_MAX},   {DSMIXBIN_FRONT_RIGHT, DSBVOLUME_MAX},
      {DSMIXBIN_FRONT_CENTER, DSBVOLUME_MIN}, {DSMIXBIN_BACK_LEFT, DSBVOLUME_MIN},
      {DSMIXBIN_BACK_RIGHT, DSBVOLUME_MIN},   {DSMIXBIN_LOW_FREQUENCY, DSBVOLUME_MIN}};
...
void CCdgChatter::Shutdown()
{
  ...
  if (m_pOutputStream)
  {
    ...
    do
    {
      m_pOutputStream->GetStatus(&dwStatus);
...
void CCdgChatter::LoadSettings()
{
  ...
  if (iPercent > 100)
    iPercent = 100;
  float fHardwareVolume =
      ((float)iPercent) / 100.0f * (VOLUME_MAXIMUM - VOLUME_MINIMUM) + VOLUME_MINIMUM;
...
HRESULT CCdgChatter::ProcessVoice(PFNCDGVOICEDATACALLBACK pfnVoiceDataCallback,
                                  VOID* pCallbackContext)
{
  CSingleLock lock(m_CritSection);
  if (!pfnVoiceDataCallback || !m_pbCompletedPackets)
    return S_OK;
...
inline HRESULT CCdgChatter::SubmitStreamPacket(DWORD dwIndex)
{
  XMEDIAPACKET xmp;
...
inline HRESULT CCdgChatter::SubmitMicrophonePacket(XMEDIAPACKET* pPacket)
{
...
HRESULT CCdgChatter::MicrophonePacketCallback(LPVOID pPacketContext, DWORD dwStatus)
{
  ...
  {
    assert(dwStatus == XMEDIAPACKET_STATUS_FAILURE || dwStatus == XMEDIAPACKET_STATUS_FLUSHED);
...
HRESULT CCdgVoiceManager::CheckDeviceChanges()
{
  ...
  // in one frame
  XGetDeviceChanges(XDEVICE_TYPE_VOICE_MICROPHONE, &dwMicrophoneInsertions, &dwMicrophoneRemovals);
  XGetDeviceChanges(XDEVICE_TYPE_VOICE_HEADPHONE, &dwHeadphoneInsertions, &dwHeadphoneRemovals);
  XGetDeviceChanges(XDEVICE_TYPE_HIGHFIDELITY_MICROPHONE, &dwHiFiMicrophoneInsertions,
  ...
  for (WORD i = 0; i < XGetPortCount(); i++)
  {
    ...
    if (m_dwConnectedCommunicators & (1 << i) &&
        ((dwMicrophoneRemovals & (1 << i)) || (dwHeadphoneRemovals & (1 << i))))
...
{
  if (dwPort < 0 || dwPort >= XGetPortCount())
    return E_FAIL;
  ...
  if (m_cfg.pfnVoiceDeviceCallback)
    m_cfg.pfnVoiceDeviceCallback(dwPort, DeviceType, CDG_VOICE_DEVICE_INSERTED,
...
HRESULT CCdgVoiceManager::OnVoiceDeviceRemoved(DWORD dwPort, CDG_DEVICE_TYPE DeviceType)
{
  ...
  if (m_cfg.pfnVoiceDeviceCallback)
    m_cfg.pfnVoiceDeviceCallback(dwPort, DeviceType, CDG_VOICE_DEVICE_REMOVED,
                                 m_cfg.pCallbackContext);
  return S_OK;
}
