                                 ...
                                 EClientQuirks quirks)
{
  ...
  object.m_People.artists.Add(
      StringUtils::Join(!tag.GetAlbumArtist().empty() ? tag.GetAlbumArtist() : tag.GetArtist(),
                        g_advancedSettings.m_musicItemSeparator)
          .c_str(),
      "AlbumArtist");
  if (tag.GetAlbumArtist().empty())
    object.m_Creator =
        StringUtils::Join(tag.GetArtist(), g_advancedSettings.m_musicItemSeparator).c_str();
  else
    object.m_Creator =
        StringUtils::Join(tag.GetAlbumArtist(), g_advancedSettings.m_musicItemSeparator).c_str();
                                 ...
                                 EClientQuirks quirks)
{
  ...
  if (tag.m_iDbId != -1)
  {
    if (!tag.m_artist.empty())
    {
      ...
      object.m_Creator =
          StringUtils::Join(tag.m_artist, g_advancedSettings.m_videoItemSeparator).c_str();
  ...
  object.m_People.director =
      StringUtils::Join(tag.m_director, g_advancedSettings.m_videoItemSeparator).c_str();
                             ...
                             CUPnPServer* upnp_server /* = NULL */)
{
  ...
  if (!item.m_bIsFolder)
  {
    ...
    if (upnp_server)
    {
      upnp_server->AddSafeResourceUri(object, ips, file_path,
        ...
        case MUSICDATABASEDIRECTORY::NODE_TYPE_ARTIST:
        {
          ...
          if (tag)
          {
            container->m_People.artists.Add(
                ...
                "Performer");
            container->m_People.artists.Add(
                CorrectAllItemsSortHack(StringUtils::Join(!tag->GetAlbumArtist().empty()
                                                              ? tag->GetAlbumArtist()
                                                              : tag->GetArtist(),
                                                          g_advancedSettings.m_musicItemSeparator))
                    .c_str(),
    ...
    else if (item.IsVideoDb())
    {
      ...
      switch (node)
      {
        ...
        case VIDEODATABASEDIRECTORY::NODE_TYPE_TITLE_TVSHOWS:
          ...
          container->m_Recorded.program_title =
              "S" + ("0" + NPT_String::FromInteger(tag.m_iSeason)).Right(2);
          container->m_Recorded.program_title +=
              "E" + ("0" + NPT_String::FromInteger(tag.m_iEpisode)).Right(2);
          container->m_Recorded.program_title +=
              StringUtils::Format(" : %s", tag.m_strTitle).c_str();
          ...
          container->m_Title =
              container->m_Recorded.series_title + " - " + container->m_Recorded.program_title;
          container->m_Title = tag.m_strTitle.c_str();
  ...
  if (item.HasArt("thumb") && upnp_server)
  {
    object->m_ExtraInfo.album_art_uri = upnp_server->BuildSafeResourceUri(
        (*ips.GetFirstItem()).ToString(), item.GetArt("thumb").c_str());
  ...
  if (upnp_server)
  {
    ...
    {
      upnp_server->AddSafeResourceUri(object, ips, item.GetArt("fanart").c_str(),
                                      "xbmc.org:*:fanart:*");
    }
  }
                          ...
                          PLT_MediaItemResource* resource /* = NULL */)
{
  ...
  tag.m_director = StringUtils::Split((CStdString)object.m_People.director,
