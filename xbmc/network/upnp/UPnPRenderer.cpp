...
+---------------------------------------------------------------------*/
NPT_Result CUPnPRenderer::SetupServices(PLT_DeviceData& data)
{
...
+---------------------------------------------------------------------*/
NPT_Result CUPnPRenderer::ProcessHttpRequest(NPT_HttpRequest& request,
...
void CUPnPRenderer::UpdateState()
{
  ...
  if (g_application.m_pPlayer->IsPlaying() || g_application.m_pPlayer->IsPaused())
  {
    ...
    avt->SetStateVariable("TransportPlaySpeed", (const char*)NPT_String::FromInteger(
                                                    g_application.m_pPlayer->GetPlaySpeed()));
    ...
    buffer = StringUtils::SecondsToTimeString((long)g_infoManager.GetTotalPlayTime(),
...
NPT_Result CUPnPRenderer::GetMetadata(NPT_String& meta)
{
  ...
  if (object)
  {
    ...
    object->m_ExtraInfo.album_art_uri =
        NPT_HttpUrl(ip, m_URLDescription.GetPort(), "/thumb.jpg", query.ToString()).ToString();
...
NPT_Result CUPnPRenderer::OnNext(PLT_ActionReference& action)
{
  ...
  {
    CApplicationMessenger::Get().SendMsg(TMSG_GUI_ACTION, WINDOW_SLIDESHOW, -1,
...
+---------------------------------------------------------------------*/
NPT_Result CUPnPRenderer::PlayMedia(const char* uri, const char* meta, PLT_Action* action)
{
  ...
  {
    bImageFile =
        NPT_String(PLT_MediaObject::GetUPnPClass(uri)).StartsWith("object.item.imageItem", true);
