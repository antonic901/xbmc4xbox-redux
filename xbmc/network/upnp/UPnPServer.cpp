...
+---------------------------------------------------------------------*/
NPT_String CUPnPServer::BuildSafeResourceUri(const char* host, const char* file_path)
{
  CURL url(file_path);
  CStdString md5;
                                               ...
                                               const PLT_HttpRequestContext& context)
{
  ...
  CLog::Log(LOGINFO, "Received UPnP Browse DirectChildren request for object '%s'",
            (const char*)object_id);
                                      ...
                                      const char* parent_id /* = NULL */)
{
  ...
  CLog::Log(LOGDEBUG, "Building UPnP response with filter '%s', starting @ %d with %d requested",
            (const char*)filter, starting_index, requested_count);
  ...
  NPT_UInt32 max_count = (requested_count == 0) ? m_MaxReturnedItems
                                                : min((unsigned long)requested_count,
                                                      (unsigned long)m_MaxReturnedItems);
  NPT_UInt32 stop_index = min((unsigned long)(starting_index + max_count),
                                          ...
                                          const PLT_HttpRequestContext& context)
{
  CLog::Log(LOGDEBUG, "Received Search request for object '%s' with search '%s'",
            (const char*)object_id, (const char*)search_criteria);
  ...
  else if (NPT_String(search_criteria).Find("object.item.audioItem") >= 0)
  {
    ...
    if (genre.GetLength() > 0)
    {
      ...
      CStdString strPath;
      strPath.Format("musicdb://genres/%ld/%ld/%ld/", database.GetGenreByName((const char*)genre),
    ...
    else if (artist.GetLength() > 0)
    {
      ...
      CStdString strPath;
      strPath.Format("musicdb://artists/%ld/%ld/", database.GetArtistByName((const char*)artist),
                     database.GetAlbumByName((const char*)album)); // will return -1 if no album
    ...
    else if (album.GetLength() > 0)
    {
      ...
      CStdString strPath;
      strPath.Format("musicdb://albums/%ld/", database.GetAlbumByName((const char*)album));
  ...
  else if (NPT_String(search_criteria).Find("object.container.album.musicAlbum") >= 0)
  {
    ...
    {
      CStdString strPath;
      strPath.Format("musicdb://genres/%ld/%ld/", database.GetGenreByName((const char*)genre),
    ...
    {
      CStdString strPath;
      strPath.Format("musicdb://artists/%ld/", database.GetArtistByName((const char*)artist));
      return OnBrowseDirectChildren(action, strPath.c_str(), filter, starting_index,
                                    requested_count, sort_criteria, context);
    }
...
+---------------------------------------------------------------------*/
NPT_Result CUPnPServer::ServeFile(NPT_HttpRequest& request,
                                  ...
                                  const NPT_String& md5)
{
      ...
      file_path.Left(8).Compare("stack://", true) == 0)
  {
    ...
    for (; url; url++)
    {
      ...
      output += "\r\n";
      output += BuildSafeResourceUri(context.GetLocalAddress().GetIpAddress().ToString(), *url);
      output += "\r\n";
    }
  ...
  return PLT_MediaConnect::ServeFile(request, context, response, file_path);
}
