...
void CEventButtonState::Load()
{
  if ( m_iKeyCode == 0 )
  {
    if ( (m_mapName.length() > 0) && (m_buttonName.length() > 0) )
    {
      ...
      {
        Reset();
        CLog::Log(LOGERROR, "ES: Could not map %s : %s to a key", m_mapName.c_str(), m_buttonName.c_str());
      }
    }
  }
  else
...
/* CEventClient                                                         */
/************************************************************************/
bool CEventClient::AddPacket(boost::shared_ptr<CEventPacket> packet)
{
  if (!packet)
    return false;
  ...
  if ( packet->Size() > 1 )
  {
    ...
    if (m_seqPackets.size() == m_seqPackets[sequence]->Size())
    {
      ...
      std::vector<uint8_t>::iterator newPayloadIter = newPayload.begin();
...
bool CEventClient::OnPacketBUTTON(CEventPacket *packet)
{
  ...
  if (flags & PTB_USE_NAME)
    CLog::Log(LOGDEBUG, "EventClient: button name \"%s\" map \"%s\" %s", button.c_str(), map.c_str(),
              active ? "pressed" : "released");
  else
    CLog::Log(LOGDEBUG, "EventClient: button code %hu %s", bcode, active ? "pressed" : "released");
...
bool CEventClient::OnPacketACTION(CEventPacket *packet)
{
  ...
  default:
    CLog::Log(LOGDEBUG, "ES: Failed - ActionType: %hu ActionString: %s", actionType, actionString.c_str());
    return false;
    break;
  }
  return true;
