  ...
  else if (info.size() == 2)
  {
    ...
    if (cat.name == "string")
    {
      ...
      else if (prop.num_params() == 2)
      {
        ...
        {
          const infomap& string_bool = string_bools[i];
    ...
    if (cat.name == "integer")
    {
      ...
      {
        const infomap &integer_bool = integer_bools[i];
    ...
    else if (cat.name == "player")
    {
      ...
      if (prop.name == "process" && prop.num_params())
      {
        ...
        {
          const infomap& player_proces = player_process[i];
    ...
    {
      for (size_t i = 0; i < sizeof(addons) / sizeof(infomap); ++i)
      {
        if (prop.name == addons[i].str && prop.num_params() == 2)
          return AddMultiInfo(CGUIInfo(addons[i].val, prop.param(0), prop.param(1)));
      }
    }
    ...
    else if (cat.name == "network")
    {
      ...
      {
        const infomap& network_label = network_labels[i];
    ...
    else if (cat.name == "system")
    {
      ...
      {
        const infomap& system_label = system_labels[i];
    ...
    else if (cat.name == "container")
    {
      ...
      {
        const infomap& container_bool = container_bools[i];
      ...
      {
        const infomap& container_int = container_ints[i];
    ...
    else if (cat.name == "fanart")
    {
      ...
      {
        const infomap& fanart_label = fanart_labels[i];
    ...
    else if (cat.name == "skin")
    {
      ...
      {
        const infomap& skin_label = skin_labels[i];
    ...
    else if (cat.name == "window")
    {
      ...
      {
        const infomap& window_bool = window_bools[i];
    ...
    else if (cat.name == "control")
    {
      ...
      {
        const infomap& control_label = control_labels[i];
...
int CGUIInfoManager::TranslateListItem(const Property& cat, const Property& prop, int id, bool container)
{
  ...
  if (ret == 0)
  {
    ...
    {
      const infomap& listitem_label = listitem_labels[i];
...
std::string CGUIInfoManager::GetLabel(int info, int contextWindow, std::string *fallback) const
{
  ...
  {
    const boost::shared_ptr<CGUIListItem> item = GUIINFO::GetCurrentListItem(contextWindow);
...
bool CGUIInfoManager::GetInt(int &value, int info, int contextWindow, const CGUIListItem *item /* = nullptr */) const
{
  ...
  {
    boost::shared_ptr<CGUIListItem> itemPtr;
    if (!item)
    {
...
INFO::InfoPtr CGUIInfoManager::Register(const std::string &expression, int context)
{
  ...
  if (condition.find_first_of("|+[]!") != condition.npos)
    res = m_bools.insert(boost::make_shared<InfoExpression>(condition, context, m_refreshCounter));
  else
    res = m_bools.insert(boost::make_shared<InfoSingle>(condition, context, m_refreshCounter));
...
bool CGUIInfoManager::EvaluateBool(const std::string& expression,
                                   int contextWindow /* = 0 */,
                                   const boost::shared_ptr<CGUIListItem>& item /* = nullptr */)
{
...
int CGUIInfoManager::getIntValue(int infoNum, const CGUIListItem *item, int contextWindow)
{
  int intValue = 0;
...
bool CGUIInfoManager::GetMultiInfoBool(const CGUIInfo &info, int contextWindow, const CGUIListItem *item)
{
  ...
  else if (!m_infoProviders.GetBool(bReturn, m_currentFile, contextWindow, info))
  {
    switch (condition)
    {
      ...
      case STRING_IS_EQUAL:
        {
          ...
          if (info.GetData2() < 0) // info labels are stored with negative numbers
          {
            ...
            boost::shared_ptr<CGUIListItem> item2;
      ...
      case INTEGER_ODD:
        {
          int leftIntValue = getIntValue(info.GetData1(), item, contextWindow);
          int rightIntValue = getIntValue(info.GetData2(), item, contextWindow);
...
std::string CGUIInfoManager::GetMultiInfoLabel(const CGUIInfo &constinfo, int contextWindow, std::string *fallback) const
{
  ...
  {
    const boost::shared_ptr<CGUIListItem> item = GUIINFO::GetCurrentListItem(
...
void CGUIInfoManager::Clear()
{
  ...
  {
    swapList.clear();
    for (std::set<INFO::InfoPtr, bool(*)(const INFO::InfoPtr&, const INFO::InfoPtr&)>::iterator it = m_bools.begin(); it != m_bools.end(); ++it)
      if (!(*it).unique())
        swapList.insert(*it);
...
void CGUIInfoManager::OnApplicationMessage(KODI::MESSAGING::ThreadMessage* pMsg)
{
  ...
  case TMSG_GUI_INFOLABEL:
  {
    if (pMsg->lpVoid)
    {
      ...
      for (std::vector<std::string>::const_iterator it = pMsg->params.begin(); it != pMsg->params.end(); ++it)
        infoLabels->push_back(GetLabel(TranslateString(*it), DEFAULT_CONTEXT));
    }
  }
  break;
  ...
  case TMSG_GUI_INFOBOOL:
  {
    if (pMsg->lpVoid)
    {
      ...
      for (std::vector<std::string>::const_iterator it = pMsg->params.begin(); it != pMsg->params.end(); ++it)
        infoLabels->push_back(EvaluateBool(*it, DEFAULT_CONTEXT));
    }
  }
  break;
