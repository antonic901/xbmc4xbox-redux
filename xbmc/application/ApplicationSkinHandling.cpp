...
bool CApplicationSkinHandling::LoadSkin(const std::string& skinID)
{
  ...
  bool previousRenderingState = false;
  ...
  CLog::Log(LOGDEBUG, "Load Skin XML: %.2f ms", XbmcThreads::SystemClockMillis() - start);
...
{
  if (g_SkinInfo != NULL && m_saveSkinOnUnloading)
...
bool CApplicationSkinHandling::LoadCustomWindows()
{
  ...
  for (std::vector<std::string>::const_iterator skinPath = vecSkinPath.begin(); skinPath != vecSkinPath.end(); ++skinPath)
  {
    ...
    if (XFILE::CDirectory::GetDirectory(*skinPath, items, ".xml", XFILE::DIR_FLAG_NO_FILE_DIRS))
    {
      for (int i = 0; i < items.Size(); ++i)
      {
        ...
        if (StringUtils::StartsWithNoCase(skinFile, "custom"))
        {
          ...
          {
            CLog::Log(LOGERROR, "Unable to load custom window XML %s. Line %i\n%s", item->GetPath().c_str(),
          ...
          {
            DialogModalityType modality = MODAL;
            hasVisibleCondition = pRootElement->FirstChildElement("visible") != NULL;
...
void CApplicationSkinHandling::ReloadSkin(bool confirm)
{
  ...
  else
  {
    ...
    std::string defaultSkin = boost::static_pointer_cast<CSettingString>(setting)->GetDefault();
...
bool CApplicationSkinHandling::OnSettingChanged(const CSetting& setting)
{
      ...
      settingId == CSettings::SETTING_LOOKANDFEEL_SKINCOLORS)
  {
    ...
    else if (settingId == CSettings::SETTING_LOOKANDFEEL_SKINTHEME)
    {
      ...
      boost::shared_ptr<CSettingString> skinFontSetting = boost::static_pointer_cast<CSettingString>(
          CServiceBroker::GetSettingsComponent()->GetSettings()->GetSetting(
