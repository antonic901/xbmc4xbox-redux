...
void CApplicationPowerHandling::ResetScreenSaver()
{
  ...
  // screen saver timer is reset only if we're not already in screensaver
  if (!m_screensaverActive && m_iScreenSaveLock == 0)
...
{
  return m_screensaverActive && WakeUpScreenSaver();
}

bool CApplicationPowerHandling::WakeUpScreenSaver()
{
