...
access_violation::access_violation(const EXCEPTION_RECORD& info)
  : win32_exception(info, "access_voilation"),
    mAccessType(Invalid),
    mBadAddress(0)
{
...
void access_violation::LogThrowMessage(const char* prefix) const
{
  if (prefix)
    if (mAccessType == Write)
      LOG(LOGERROR, "%s : %s at 0x%08x: Writing location 0x%08x", prefix, what(), where(),
          address());
    else if (mAccessType == Read)
      LOG(LOGERROR, "%s : %s at 0x%08x: Reading location 0x%08x", prefix, what(), where(),
          address());
    else if (mAccessType == DEP)
      LOG(LOGERROR, "%s : %s at 0x%08x: DEP violation, location 0x%08x", prefix, what(), where(),
          address());
    else
      LOG(LOGERROR, "%s : %s at 0x%08x: unknown access type, location 0x%08x", prefix, what(),
          where(), address());
