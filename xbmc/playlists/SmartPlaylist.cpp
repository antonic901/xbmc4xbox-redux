...
int CSmartPlaylistRule::TranslateField(const char* field) const
{
  for (unsigned int i = 0; i < NUM_FIELDS; i++)
    if (StringUtils::EqualsNoCase(field, fields[i].string))
...
std::string CSmartPlaylistRule::GetLocalizedField(int field)
{
  for (unsigned int i = 0; i < NUM_FIELDS; i++)
    if (field == fields[i].field)
      return g_localizeStrings.Get(fields[i].localizedString);
  ...
  else if (strType == "artists")
  {
    ...
    if (m_field == FieldGenre)
    {
      query = negate +
              ...
              parameter + ")";
      query += " OR ";
      query += "EXISTS (SELECT DISTINCT album_artist.idArtist FROM album_artist, album_genre, "
               "genre WHERE album_artist.idArtist = " +
               GetField(FieldId, strType) +
               " AND album_artist.idAlbum = album_genre.idAlbum AND album_genre.idGenre = "
               "genre.idGenre AND genre.strGenre" +
               parameter + "))";
    }
    ...
    std::set<std::string>& referencedPlaylists) const
{
       ...
       it != m_combinations.end(); ++it)
  {
    ...
    boost::shared_ptr<CSmartPlaylistRuleCombination> combo =
        boost::static_pointer_cast<CSmartPlaylistRuleCombination>(*it);
    if (combo)
