  ...
  if (node != NULL)
  {
    ...
    if (element != NULL)
    {
      ...
      if (positionId != NULL && strlen(positionId) > 0)
        position = 0;
      else if ((positionId = element->Attribute(SETTING_XML_ATTR_AFTER)) != NULL &&
               strlen(positionId) > 0)
        position = 1;
...
{
  for (SettingList::const_iterator setting = m_settings.begin(); setting != m_settings.end();
       ++setting)
    delete *setting;
  m_settings.clear();
  if (m_control)
...
bool CSettingGroup::Deserialize(const TiXmlNode* node, bool update /* = false */)
{
  ...
  while (settingElement != NULL)
  {
    ...
    if (CSettingCategory::DeserializeIdentification(settingElement, settingId))
    {
      ...
      for (SettingList::iterator itSetting = m_settings.begin(); itSetting != m_settings.end();
           ++itSetting)
      {
...
{
  for (SettingList::const_iterator itSetting = settings.begin(); itSetting != settings.end();
       ++itSetting)
...
bool CSettingCategory::Deserialize(const TiXmlNode* node, bool update /* = false */)
{
  ...
  while (groupNode != NULL)
  {
    ...
    if (CSettingGroup::DeserializeIdentification(groupNode, groupId))
    {
      ...
      for (SettingGroupList::iterator itGroup = m_groups.begin(); itGroup != m_groups.end();
           ++itGroup)
      {
...
{
  for (SettingGroupList::const_iterator itGroup = groups.begin(); itGroup != groups.end();
       ++itGroup)
...
{
  for (SettingCategoryList::const_iterator it = m_categories.begin(); it != m_categories.end();
       ++it)
    delete *it;
...
bool CSettingSection::Deserialize(const TiXmlNode* node, bool update /* = false */)
{
  ...
  while (categoryNode != NULL)
  {
    ...
    if (CSettingCategory::DeserializeIdentification(categoryNode, categoryId))
    {
      ...
      for (SettingCategoryList::iterator itCategory = m_categories.begin();
           itCategory != m_categories.end(); ++itCategory)
      {
...
{
  for (SettingCategoryList::const_iterator itCategory = categories.begin();
