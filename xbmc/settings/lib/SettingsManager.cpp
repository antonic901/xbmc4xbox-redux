...
CSettingsManager::CSettingsManager() : m_initialized(false), m_loaded(false)
{
}
...
bool CSettingsManager::Save(TiXmlNode* root) const
{
  ...
  for (std::set<ISubSettings*>::const_iterator it = m_subSettings.begin();
       it != m_subSettings.end(); ++it)
  {
    if (!(*it)->Save(root))
      return false;
  }
...
void CSettingsManager::Clear()
{
  ...
  m_settings.clear();
  for (SettingSectionMap::iterator section = m_sections.begin(); section != m_sections.end();
       ++section)
...
void CSettingsManager::SetInitialized()
{
  ...
  for (SettingMap::iterator itSettingDep = m_settings.begin(); itSettingDep != m_settings.end();
       ++itSettingDep)
  {
    ...
    for (SettingDependencies::const_iterator depIt = deps.begin(); depIt != deps.end(); ++depIt)
    {
      ...
      for (std::set<std::string>::const_iterator itSettingId = settingIds.begin();
           itSettingId != settingIds.end(); ++itSettingId)
      {
        ...
        for (SettingDependencies::const_iterator itDeps = settingDeps.begin();
             itDeps != settingDeps.end(); ++itDeps)
        {
...
void CSettingsManager::AddSection(CSettingSection* section)
{
  ...
  for (SettingCategoryList::const_iterator categoryIt = section->GetCategories().begin();
       categoryIt != section->GetCategories().end(); ++categoryIt)
  {
    (*categoryIt)->CheckRequirements();
    for (SettingGroupList::const_iterator groupIt = (*categoryIt)->GetGroups().begin();
         groupIt != (*categoryIt)->GetGroups().end(); ++groupIt)
    {
      (*groupIt)->CheckRequirements();
      for (SettingList::const_iterator settingIt = (*groupIt)->GetSettings().begin();
           settingIt != (*groupIt)->GetSettings().end(); ++settingIt)
      {
        ...
        if (setting == m_settings.end())
        {
          ...
          std::pair<SettingMap::iterator, bool> tmpIt =
              m_settings.insert(make_pair(settingId, tmpSetting));
          setting = tmpIt.first;
        }
                                        ...
                                        const std::set<std::string>& settingList)
{
  ...
  for (std::set<std::string>::const_iterator settingIt = settingList.begin();
       settingIt != settingList.end(); ++settingIt)
  {
...
{
  CExclusiveLock lock(m_settingsCritical);
  for (SettingMap::iterator settingIt = m_settings.begin(); settingIt != m_settings.end();
       ++settingIt)
...
void CSettingsManager::UnregisterSettingsHandler(ISettingsHandler* settingsHandler)
{
  ...
  CExclusiveLock lock(m_critical);
  SettingsHandlers::iterator it =
      find(m_settingsHandlers.begin(), m_settingsHandlers.end(), settingsHandler);
...
std::vector<CSettingSection*> CSettingsManager::GetSections() const
{
  ...
  for (SettingSectionMap::const_iterator sectionIt = m_sections.begin();
...
bool CSettingsManager::SetList(const std::string& id,
                               const std::vector<boost::shared_ptr<CSetting> >& value)
{
  CSharedLock lock(m_settingsCritical);
  CSetting* setting = GetSetting(id);
  if (setting == NULL || setting->GetType() != SettingTypeList)
...
void CSettingsManager::AddCondition(const std::string& identifier,
...
bool CSettingsManager::Serialize(TiXmlNode* parent) const
{
  ...
  for (SettingMap::const_iterator it = m_settings.begin(); it != m_settings.end(); ++it)
  {
    ...
    if (settingNode == NULL)
    {
      CLog::Log(LOGWARNING, "CSetting: unable to write <%s> tag in <%s>", parts.at(1).c_str(),
                parts.at(0).c_str());
      continue;
    }
...
void CSettingsManager::OnSettingChanged(const CSetting* setting)
{
  ...
  {
    for (SettingDependencies::const_iterator depIt = depsIt->second.begin();
         depIt != depsIt->second.end(); ++depIt)
...
void CSettingsManager::OnSettingPropertyChanged(const CSetting* setting, const char* propertyName)
{
  ...
  {
    for (std::set<std::string>::const_iterator childIt = settingIt->second.children.begin();
         childIt != settingIt->second.children.end(); ++childIt)
      UpdateSettingByDependency(*childIt, dependencyType);
  }
}

CSetting* CSettingsManager::CreateSetting(const std::string& settingType,
                                          const std::string& settingId,
...
{
  CSharedLock lock(m_critical);
  for (SettingsHandlers::const_iterator it = m_settingsHandlers.begin();
       it != m_settingsHandlers.end(); ++it)
  {
...
bool CSettingsManager::LoadSetting(const TiXmlNode* node, CSetting* setting, bool& updated)
{
  ...
  bool isDefault =
      isDefaultAttribute != NULL && StringUtils::EqualsNoCase(isDefaultAttribute, "true");

  if (!setting->FromString(settingElement->FirstChild() != NULL
  ...
  for (std::set<CSettingUpdate>::const_iterator update = updates.begin(); update != updates.end();
       ++update)
                                     ...
                                     const CSettingUpdate& update)
{
  ...
  if (update.GetType() == SettingUpdateTypeRename)
  {
    ...
    if (setting->FromString(oldSettingNode->FirstChild() != NULL
