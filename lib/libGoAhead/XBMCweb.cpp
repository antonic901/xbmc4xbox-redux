  ...
  xbmcNavigationHandlerType    xbmcNavigator[ ] = {
    ...
    while( xbmcNavigator[ cmd].xbmcAppStateCode != -1) {
      ...
      if( stricmp( parameter,xbmcNavigator[ cmd].xbmcNavigateParameter) == 0) {
        ...
        {
          shares = CMediaSourceSettings::GetInstance().GetSources("pictures");
        ...
        {
          shares = CMediaSourceSettings::GetInstance().GetSources("files");
        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              {
                CServiceBroker::GetAppMessenger()->PostMsg(TMSG_PICTURE_SHOW, -1, -1, NULL, itm->GetPath());
              }
            }
          }
        }
