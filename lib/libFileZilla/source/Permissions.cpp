...
int CPermissions::GetDirectoryListing(LPCTSTR user, std::string dir, t_dirlisting *&pResult)
{
    unsigned int index;
...
int CPermissions::GetDirName(LPCTSTR user, std::string dirname, std::string currentdir, int op, std::string &physical, std::string &logical)
{
    ...
    int remove=0; //Number of pieces that will be removed due to dots
        for (std::list<std::string>::reverse_iterator iter=piecelist.rbegin(); iter!=piecelist.rend(); iter++)
        {
            std::string tmp=*iter;
            if (tmp == "..")
  ...
  std::string realdir, realdirname;
  std::string dir2(dir), dirname2(dirname);
...
int CPermissions::GetFileName(LPCTSTR user, std::string filename, std::string currentdir, int op, std::string &result)
{
    //Get userindex based on user string
    unsigned int index;
    for (index=0; index<m_UsersList.size(); index++)
...
std::string CPermissions::GetHomeDir(const CUser &user, BOOL bRealPath /*=FALSE*/) const
{
...
std::string CPermissions::GetHomeDir(LPCTSTR username, BOOL bRealPath /*=FALSE*/) const
{
    CUser user;
...
std::string CPermissions::GetHomeDir(unsigned int index, BOOL bRealPath /*=FALSE*/) const
{
...
int CPermissions::GetRealDirectory(std::string directory, int user, t_directory &ret, BOOL &truematch)
{
    ...
    if (!bRelative)
    {
        ...
        std::string path = PathPieces.front();
    ...
    else
    {
        ...
        std::string homepath = GetHomeDir(user, TRUE);
...
int CPermissions::ChangeCurrentDir(LPCTSTR user, std::string &currentdir, std::string &dir)
{
    //Get userindex based on user string
    unsigned int index;
    for (index=0; index<m_UsersList.size(); index++)
...
std::string CPermissions::GetShortcutTarget(LPCTSTR filename)
{
#if !defined(_XBOX)
    CoInitialize(0);
...
BOOL CPermissions::GetUser(std::string user, CUser &userdata) const
{
...
int CPermissions::GetShortDirectoryListing(LPCTSTR user, std::string currentDir, std::string dirToDisplay, t_dirlisting *&pResult)
{
    unsigned int index;
    for (index=0; index<m_UsersList.size(); index++)
    {
