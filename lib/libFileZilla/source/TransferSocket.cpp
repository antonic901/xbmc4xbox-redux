...
void CTransferSocket::OnReceive(int nErrorCode)
{
    ...
    if (m_nMode==TRANSFERMODE_RECEIVE)
    {
      ...
      if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool("services.ftpautofatx"))
      {
        ...
        std::string strFilename = URIUtils::GetFileName(m_Filename);
        std::string strPath;
        URIUtils::GetDirectory(m_Filename,strPath);
        std::vector<std::string> tokens;
        StringUtils::Tokenize(strPath,tokens,"\\/");
        strPath = tokens.front();
        for (vector<std::string>::iterator iter=tokens.begin()+1; iter != tokens.end(); ++iter)
        {
