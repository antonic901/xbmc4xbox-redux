...
std::map<std::string, int> CServerThread::m_userIPs;
...
void CServerThread::AddNewSocket(SOCKET sockethandle)
{
  ...
  memset(op, 0, sizeof(t_connop));
    ...
    std::string msg = m_pOptions->GetOption(OPTION_WELCOMEMESSAGE);
...
int CServerThread::GetIpCount(const std::string &ip) const
{
    int count=0;
    EGCS;
...
void CServerThread::IncIpCount(const std::string &ip)
{
    int count;
    EGCS;
    std::map<std::string, int>::iterator iter=m_userIPs.find(ip);
...
void CServerThread::DecIpCount(const std::string &ip)
{
    EGCS;
    std::map<std::string, int>::iterator iter=m_userIPs.find(ip);
...
std::string CServerThread::GetExternalIP()
{
    std::string ip;
    m_threadsync.Lock();
