...
#if defined(_XBOX)
std::string GetDefaultWelcomeMessage()
{
  return "\r\n---------------------------------------------------------------------------"
         "\r\n%v\r\n"
...
void COptions::SetOption(int nOptionID, LPCTSTR value)
{
    ...
    case OPTION_ADMINIPBINDINGS:
        {
            std::string sub;
            std::list<std::string> ipBindList;
            ...
            str = "";
            for (std::list<std::string>::iterator iter = ipBindList.begin(); iter!=ipBindList.end(); iter++)
                if (*iter != "127.0.0.1")
                    str += *iter + " ";
...
void COptions::Init()
{
    ...
    if (xml.Load(buffer))
    {
        if (xml.FindElem( _T("FileZillaServer") ))
        {
            ...
            {
                std::string value=xml.GetChildData();
                std::string name=xml.GetChildAttrib( _T("name") );
